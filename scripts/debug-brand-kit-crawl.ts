...
import { mergeFontFamilies } from "../src/lib/brandKit/crawl/color-utils";
import { buildCopyCorpus } from "../src/lib/brandKit/crawl/copy-corpus";
import { extractOnelinerDeterministic } from "../src/lib/brandKit/crawl/copy-extract";
import {
  brandNameFromPage,
  cssVarColors,
  fontFaceFamilies,
  ...
  logoFromJsonLd,
  themeColorMeta,
} from "../src/lib/brandKit/crawl/parsers";
import { fetchCrawlPages } from "../src/lib/brandKit/crawl/run-crawl";
import { buildPaletteValue, buildTypographyValue, rankLogoCandidates, shouldAutoResolveLogo } from "../src/lib/brandKit/crawl/scoring";
import { normalizeBrandKitUrlInput } from "../src/lib/brandKit/crawl/url-utils";

async function main() {
  const raw = process.argv[2]?.trim() || "coca-cola.com/es/es";
  const normalized = normalizeBrandKitUrlInput(raw);
  if (!normalized.ok) {
