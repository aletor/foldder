...
function shouldClearPersistedTheme(doc: GenomaDocument): boolean {
  const slot = doc.slots.palette;
  return slot.status === "empty" || (slot.status === "pending" && !slot.value);
}

export function useGenomaTheme(doc: GenomaDocument): GenomaThemeState {
