...
import { deriveBrandThemeFromDoc } from "@/lib/genoma/brand-theme-color";
import { useGenomaTheme } from "./use-genoma-theme";
import { renderHook } from "@testing-library/react";

describe("useGenomaTheme", () => {
  it("conserva el último tema cuando palette pasa a candidates", () => {
    const resolved = createEmptyGenoma();
    ...
    const { result, rerender } = renderHook(({ doc }) => useGenomaTheme(doc), {
  ...
  it("vuelve a neutro en genoma vacío", () => {
    const resolved = createEmptyGenoma();
    resolved.slots.palette = {
      ...resolved.slots.palette,
      status: "resolved",
    ...
    rerender({ doc: createEmptyGenoma() });
