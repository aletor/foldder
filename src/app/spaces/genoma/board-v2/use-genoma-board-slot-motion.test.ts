...
import type { SlotState } from "@/lib/genoma/genoma-types";
import {
  buildInitialMotionStates,
  computeSlotDisplayPhase,
  detectSlotMotionTransition,
} from "./use-genoma-board-slot-motion";
...
describe("use-genoma-board-slot-motion", () => {
  ...
  it("buildInitialMotionStates idle con genoma resuelto", () => {
    const doc = createEmptyGenoma();
    doc.slots.logo = slot({
      status: "resolved",
