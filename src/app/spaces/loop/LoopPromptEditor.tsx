...
export interface LoopPromptEditorProps {
  /** Valor con tokens `{fieldKey}` (fuente de verdad en datos de Loop). */
  value: string;
...
function applyChipStyle(
  ...
  span.className = `loop-chip${field ? "" : " loop-chip--invalid"}`;
...
export function LoopPromptEditor({
  value,
  fields,
  insertableFields,
  label = "Prompt",
  ...
  return (
    <div className="loop-prompt-editor-wrap">
      <div className="loop-template-panel__row">
        <span className="loop-template-panel__label">{label}</span>
        <select
      ...
      <div
        ref={editorRef}
        className="loop-prompt-editor nodrag"
        contentEditable
