/**
 * Loop — modelo compartido para orquestar nodos creativos sobre un Dataset.
 *
 ...
 * No hay un "modo Loop" en el nodo: solo se añaden bindings opcionales.
 */
...
export interface LoopInputBinding {
  ...
  optionsFrom?: LoopManualOptionsFrom;
}

/** Mapa inputId → binding, guardado en node.data[LOOP_BINDINGS_KEY]. */
export type LoopBindings = Record<string, LoopInputBinding>;

/** Tipo lógico de un input creativo de cara a Loop. */
...
/** Estado de una ejecución de Loop (preview o lote). */
export type LoopRunStatus = "idle" | "preview" | "running" | "done" | "partial" | "error";
...
export interface LoopNodeData {
  ...
  spaceId?: string;
  /**
   * Plantilla gobernada por Loop (no por el nodo creativo):
   * prompt con tokens {campo} editable dentro de Loop. `undefined` = aún no
   * sembrado desde el nodo creativo conectado (el editor lo inicializa una vez).
   */
  templatePrompt?: string;
  /** Bindings por input (fijo / columna del Dataset), gobernados por Loop. */
   ...
   */
  datasetOutputsByChannel?: Record<string, LoopDatasetOutputSettings>;
  ...
  /**
   * Modo 2 (plantilla Designer): mapeo hueco→columna del Dataset de Loop.
...
export interface LoopDatasetOutputSettings {
  enabled: boolean;
  columnLabel: string;
...
/** Traduce el tipo de handle de un input a su tipo lógico para Loop. */
