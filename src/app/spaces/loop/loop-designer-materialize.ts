/**
 * Loop — materialización CONGELADA de un Designer por fila (modo `node-clone`).
 *
 ...
 * clona (ids nuevos) → rellena huecos PENDIENTES con el mapeo de Loop → resuelve enlaces de esa
 * fila → elimina enlaces → re-estampa `slideKey`.
 *
 * `slotColumnMap` (opcional) viene de la UI de mapeo de Loop (Modo 2). Los huecos sin columna
...
export function freezeDesignerPagesForRow(
  ...
  return templatePages.map((tpl) => {
    ...
    // Las instancias Loop no participan del modo bucle intra-nodo.
...
function designerRowNodeId(loopId: string, rowIndex: number): string {
  return `loop_${loopId}_r${rowIndex}_designer`;
}
...
export function buildDesignerRowNode(
  ...
  return {
    id: designerRowNodeId(loopId, row.rowIndex),
    type: "designer",
...
export function buildDesignerGeneratedSubgraph(
  ...
  const nodes: Node[] = rows.map((row, index) =>
    buildDesignerRowNode(loopId, row, 80 + index * DESIGNER_ROW_GAP_Y),
  );
  return { nodes, edges: [] };
}
