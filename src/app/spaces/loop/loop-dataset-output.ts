/**
 * Loop — escribir resultados de generación de vuelta al Dataset conectado.
 */
...
export function suggestLoopOutputColumnLabel(templateLabel: string): string {
...
export interface ApplyLoopDatasetOutputResult {
  dataset: Dataset;
  fieldId: string;
  fieldKey: string;
  fieldLabel: string;
...
export function applyLoopResultsToDataset(args: {
  dataset: Dataset;
  listId: string;
  rows: MaterializedRow[];
  settings: LoopDatasetOutputSettings;
}): ApplyLoopDatasetOutputResult {
...
export interface LoopChannelOutput {
...
export interface ApplyLoopChannelResult extends ApplyLoopDatasetOutputResult {
  channelId: string;
}

export interface ApplyLoopChannelsResult {
  dataset: Dataset;
  channels: ApplyLoopChannelResult[];
  totalWritten: number;
  totalSkipped: number;
 ...
 *
 * Encadena el Dataset entre canales (cada `applyLoopResultsToDataset` parte del resultado
 ...
 */
export function applyLoopChannelsToDataset(args: {
  dataset: Dataset;
  listId: string;
  channels: LoopChannelOutput[];
}): ApplyLoopChannelsResult {
  const { listId } = args;
  let dataset = args.dataset;
  const channels: ApplyLoopChannelResult[] = [];
  let totalWritten = 0;
  let totalSkipped = 0;
  ...
  for (const ch of args.channels) {
    ...
    const res = applyLoopResultsToDataset({
      dataset,
      listId,
      rows: ch.rows,
