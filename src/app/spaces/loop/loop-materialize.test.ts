...
describe("loop-materialize", () => {
  it("builds an autonomous subgraph per row (nano + prompt + refs)", () => {
    ...
    expect(nano?.id).toBe("loop_pop1_r0_nano");
  ...
  it("multi-canal: dispone carriles por canal con IDs únicos y reconocibles", () => {
    ...
    // IDs reconocibles por el reconciliador del nested space (prefijo loop_<id>_r).
    ...
    const c0 = nodes.find((n) => n.id === "loop_pop1_r0_c0_nano")!;
    const c1 = nodes.find((n) => n.id === "loop_pop1_r0_c1_nano")!;
