...
export interface LoopTemplatePanelProps {
  /** Prompt plantilla (texto fijo + tokens {campo}); editado dentro de Loop. */
  promptText: string;
...
/**
 * Editor de plantilla que vive DENTRO de Loop. Muestra el prompt que se enviará
 ...
 */
export function LoopTemplatePanel({
  promptText,
  bindings,
  schema,
  constantFields,
  ...
  return (
    <div
      className="loop-template-panel nodrag nopan"
      onPointerDown={(e) => e.stopPropagation()}
    >
      <div className="loop-template-panel__head">
        <span className="loop-template-panel__title">Plantilla</span>
        <span className="loop-template-panel__hint">se envía al nodo creativo</span>
      </div>

      <div className="loop-template-panel__field">
        <LoopPromptEditor
          value={promptText}
      ...
      {imageSlots.length > 0 ? (
        <div className="loop-template-panel__refs">
          <span className="loop-template-panel__refs-head">
            ...
            const current =
              binding?.source === "manual"
                ...
                : binding?.source === "column"
                  ...
                  : FIXED_VALUE;
            return (
              <label key={slot.inputId} className="loop-template-panel__ref">
                <span className="loop-template-panel__ref-label">{slot.label}</span>
                {slot.sourceLabel ? (
                  <span className="loop-template-panel__ref-connected" title="Cable activo en Image Creation">
      ...
      {manualSlots.length > 0 ? (
        <div className="loop-template-panel__refs">
          <span className="loop-template-panel__refs-head">Rellenar antes de generar</span>
          {manualSlots.map((slot) => {
            const binding = bindings[slot.inputId];
            return (
              <label key={slot.inputId} className="loop-template-panel__ref">
                <span className="loop-template-panel__ref-label">{binding?.manualLabel || slot.label}</span>
                <input
                  type="text"
                  className="loop-template-panel__select nodrag"
        ...
        <span className="loop-template-panel__refs-empty">
