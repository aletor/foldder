# Integración de nodos con Loop

Cómo un tipo de nodo se convierte en **plantilla** de Loop y qué superficies hay que tocar
...
> Regla de oro: **Loop no conoce los nodos por hardcode**. Los descubre por la _declaración de
> orquestación_ que cada nodo expone en `NODE_REGISTRY[type].orchestration`
> (`loop-declaration.ts`). Lo específico de cada nodo vive en su registro y en sus helpers; el
...
`loop-declaration.ts` → `getNodeOrchestrationDeclaration(type)`:

| Modo | Quién | Qué hace Loop | Resultado |
| --- | --- | --- | --- |
...
- **Handles de Loop** (`LoopNode.tsx` → `HANDLES`): `dataset` (target, izq.), `template`
  (target, izq.), `media_list` y `out` (source, der.).
- **Enlace plantilla**: `loop-template-link.ts` → `findLoopTemplateLinkEdge` resuelve el
  nodo conectado al handle `template`.
- **Designer (node-clone)**: `loop-designer-template.ts` →
  `resolveDesignerTemplateConfig(loopId, nodes, edges)` devuelve `{ pages, dynamicFields,
...
- **Regla de exclusión Modo 1 / Modo 2** (`connection-utils.ts`): un Designer no puede tener a la
  vez un Dataset directo (`DESIGNER_DATASET_INPUT_HANDLE`) y ser plantilla de Loop
  (`LOOP_TEMPLATE_INPUT_HANDLE`). `designerModeConflictReason` lo bloquea en `onConnect` /
...
Componente: `LoopStudio.tsx` (3 columnas: slots clicables → centro con picker → resumen).

### input-binding (Image Creation)
- **Slots**: `loop-studio-summary.ts` → `buildLoopStudioSlots` (prompt, tokens del prompt,
  refs de imagen).
- **Mapeo**: tokens `{columna}` en el prompt (`loop-tokens.ts`) + bindings de refs
  (`LoopBindings`/`LoopInputBinding` en `loop-types.ts`, refs activas en
  `loop-active-refs.ts`).
- **Resumen/validación**: `buildLoopStudioSummary`.
- **Resolución por fila**: `loop-resolve.ts` (`resolvePromptForRow`, `resolveImageBindingForRow`).
- **Generación**: `loop-generate.ts` (`generateLoopImage`) + materialización
  `loop-materialize.ts` (`buildRowSubgraph`, `buildGeneratedSubgraph`, `buildMediaListOutput`).
...
- **Descubrimiento**: `loop-designer-fields.ts` → `extractDesignerDynamicFields(pages)` →
  `DesignerDynamicField[]` (`status: "bound" | "pending"`).
- **Mapeo hueco→columna** (solo `pending`): vive en Loop, no en el objeto.
  `LoopNode.onChangeDesignerSlotBinding` → `nodeData.designerSlotBindings`
...
- **Materialización**: `loop-designer-materialize.ts` → `freezeDesignerPagesForRow(pages,
...
### input-binding (Image Creation)
- Modelo: `loop-form.ts` → `deriveLoopForm` (campo por token del prompt + por ref ligada a
  columna). Resolución: `resolveFormPrompt` + `resolveFormImages`. Autorelleno:
  `autofillFormFromRow`. UI: `LoopFormPanel.tsx`. Genera **1** imagen.

### node-clone (Designer)
- Modelo: `loop-designer-form.ts` → `deriveDesignerForm({ dynamicFields, slotBindings, dataset,
...
- Resolución: `resolveDesignerSlotValues` → `DesignerSlotValueMap` (no depende del Dataset: las
  ...
  slotValues)`.
- Generación: `LoopNode.onGenerateDesignerForm` congela 1 instancia, la **rasteriza** (driver
...
Crear/actualizar: `LoopNode.onShareForm` → `POST /api/loop-share` (valida payload e
idempotencia por `existingToken`). Snapshot en `LoopSharePayload` (`loop-share-types.ts`).
...
- Payload: `loop-share-payload.ts` (`formModel`, `templateModel`, refs).
- Cliente: `PublicLoopFormClient.tsx` → `POST /api/loop-share/[token]/generate` →
  `loop-share-generate.ts` (`generateLoopShareImage`, wallet/gate).
...
> si es `input-binding` con IA, reutiliza la ruta `/generate` y `loop-share-generate.ts`.
...
- Driver (request/ref + `Promise`): ver `LoopNode.rasterizeDesignerPages` y el mismo patrón en
  `PublicDesignerFormClient`.
- Orquestación lote + subida S3: `loop-designer-raster.ts` (`rasterizeAndUploadDesignerRows`,
...
UI: `LoopDatasetOutputPanel.tsx` (`variant: "image" | "designer"`), dentro del resumen del
Studio. Settings: `LoopDatasetOutputSettings` (`loop-types.ts`).

- **Imagen (1 columna)**: `loop-dataset-output.ts` + `persist-loop-dataset-output.ts`
  (`findImageFieldForOutput`, versionado/idempotencia por celda).
- **Designer (M columnas, 1 por slide)**: `loop-designer-dataset-output.ts`
  (`applyDesignerSlidesToDataset`, `makeLoopDesignerGroupId`, columnas estables por `slideKey`,
  huérfanas marcadas) + `persist-loop-designer-dataset-output.ts`. Reparto M columnas × N filas.
...
2. **Handles**: asegurar que el nodo expone una salida conectable al handle `template` de Loop.
3. Si es **input-binding**: normalmente basta la declaración. Verifica slots/summary/resolución
   (genéricos) y, si la generación difiere, añade tu `generate` (paralelo a `loop-generate.ts`).
4. Si es **node-clone**:
   ...
   - Studio: ramifica en `LoopStudio` con `isXxxTemplate` (reutiliza slots/picker/summary).
   - Formulario + público: replica el patrón Designer (`loop-designer-form.ts`,
