/**
 * Dónde inyectar el prompt del Studio / bindings en una tubería de Loop.
 *
...
/** Tipos que consumen el prompt de plantilla de Loop dentro de la tubería. */
...
function bindingInputId(key: string, binding: LoopInputBinding): string {
  return binding.inputId?.trim() || key;
}

function isImageBindingKey(key: string, binding: LoopInputBinding): boolean {
  return IMAGE_BINDING_INPUT_IDS.has(bindingInputId(key, binding));
}

function isPromptBindingKey(key: string, binding: LoopInputBinding): boolean {
...
export function adaptLoopBindingsForPipeline(
  bindings: Record<string, LoopInputBinding> | undefined,
  analysis: PipelineAnalysis,
  nodeById: Map<string, ExecutorNode>,
): Record<string, LoopInputBinding> {
  if (!bindings) return {};
  const out: Record<string, LoopInputBinding> = { ...bindings };
