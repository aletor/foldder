...
/** Sube un data URL de imagen de Loop a S3 si hace falta. */
export async function uploadLoopImageOutput(
  url: string,
  opts: { projectId: string | null; mediaId: string },
...
export async function ensureMaterializedRowsHaveStableUrls(
  ...
  for (const row of rows) {
    ...
    try {
      const uploaded = await uploadLoopImageOutput(row.output, {
        projectId,
        mediaId: `loop_${loopId}_r${row.rowIndex}`,
      });
      out.push(uploaded ? { ...row, output: uploaded.url, s3Key: uploaded.s3Key } : row);
    } catch (err) {
      console.warn("[Loop] upload row image failed", row.rowIndex, err);
      out.push(row);
    }
  }
  return out;
