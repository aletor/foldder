...
export function defaultLoopDatasetOutputSettings(templateLabel: string | null): LoopDatasetOutputSettings {
  return {
    enabled: false,
    columnLabel: suggestLoopOutputColumnLabel(templateLabel ?? "Resultado"),
    conflictStrategy: "versioned",
    fillMode: "empty_only",
  };
}

export function LoopDatasetOutputPanel({
  settings,
  schema,
  templateLabel,
  lastWriteSummary,
  ...
  const patch = (partial: Partial<LoopDatasetOutputSettings>) => {
  ...
  return (
    <div className="loop-studio-dataset-output">
      <label className="loop-studio-dataset-output__toggle">
        <input
          type="checkbox"
      ...
      {settings.enabled ? (
        <div className="loop-studio-dataset-output__body">
          {isDesigner ? (
            <p className="loop-studio-dataset-output__hint">
          ...
          <label className="loop-studio-dataset-output__field">
            <span>{isDesigner ? "Prefijo de las columnas" : "Nombre de la columna"}</span>
            <input
              type="text"
              value={settings.columnLabel}
              placeholder={suggestLoopOutputColumnLabel(templateLabel ?? "Resultado")}
              onChange={(e) => patch({ columnLabel: e.target.value })}
            />
          </label>

          {matchingField ? (
            <fieldset className="loop-studio-dataset-output__group">
              <legend>
                <RefreshCw size={12} aria-hidden /> La columna «{matchingField.label}» ya existe
              </legend>
              <label className="loop-studio-dataset-output__radio">
                <input
                  type="radio"
          ...
          {lastWriteSummary ? (
            <p className="loop-studio-dataset-output__summary">{lastWriteSummary}</p>
          ) : null}
        </div>
      ) : null}
    </div>
