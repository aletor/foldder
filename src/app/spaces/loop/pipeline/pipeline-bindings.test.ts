...
describe("pipeline-bindings — sink y namespace", () => {
  ...
  it("Background Remover puede ser sink (rgba → loop.template)", () => {
    expect(
      isValidLoopSinkEdge({
  ...
  it("Layerizer puede ser sink si tiene executor (layout → loop.template)", () => {
    expect(
      isValidLoopSinkEdge({
  ...
  it("columnOverrideForRow: manual texto devuelve valor constante", () => {
    const binding: LoopInputBinding = { inputId: "prompt", source: "manual", manualValue: "  hola  " };
  ...
  it("columnOverrideForRow: manual imagen usa la URL tecleada", () => {
    const binding: LoopInputBinding = { inputId: "image", source: "manual", manualValue: "https://x/y.png" };
  ...
  it("columnOverrideForRow: manual vacío no aporta valor", () => {
    const binding: LoopInputBinding = { inputId: "prompt", source: "manual", manualValue: "   " };
  ...
  it("columnOverrideForRow: tokens manuales se sustituyen en el prompt (constante por fila)", () => {
    const binding: LoopInputBinding = { inputId: "prompt", source: "column", listId: "l1", fieldId: "f1" };
