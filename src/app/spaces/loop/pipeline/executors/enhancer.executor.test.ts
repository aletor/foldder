...
const enhancePromptForLoop = vi.fn();
vi.mock("../transports/loop-enhance", () => ({
  enhancePromptForLoop: (args: unknown) => enhancePromptForLoop(args),
}));
...
beforeEach(() => {
  enhancePromptForLoop.mockReset();
  enhancePromptForLoop.mockResolvedValue("prompt cinematográfico, luz suave, 85mm");
});
...
describe("enhancerExecutor", () => {
  it("mejora el prompt del input de texto", async () => {
    ...
    expect(enhancePromptForLoop).toHaveBeenCalledWith({ prompt: "un coche rojo" });
  });
  ...
  it("el override tiene prioridad sobre el input conectado", async () => {
    ...
    expect(enhancePromptForLoop).toHaveBeenCalledWith({ prompt: "de la fila" });
  });
