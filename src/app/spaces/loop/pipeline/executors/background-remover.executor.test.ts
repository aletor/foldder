...
const matteImageForLoop = vi.fn();
vi.mock("../transports/loop-matte", () => ({
  matteImageForLoop: (args: unknown) => matteImageForLoop(args),
}));
...
beforeEach(() => {
  matteImageForLoop.mockReset();
  matteImageForLoop.mockResolvedValue({
...
describe("backgroundRemoverExecutor", () => {
  it("envía la imagen upstream a matte y devuelve el recorte", async () => {
    ...
    expect(matteImageForLoop).toHaveBeenCalledWith({
  ...
  it("falls back to data URL when matte has no S3 url", async () => {
    matteImageForLoop.mockResolvedValue({ rgbaImage: "data:image/png;base64,abc" });
    const out = await backgroundRemoverExecutor.execute({
