...
/**
 * Executor de Background Remover para tuberías de Loop.
 * Imagen de entrada → recorte RGBA vía `/api/spaces/matte`.
 */

import { matteImageForLoop } from "../transports/loop-matte";
import {
...
export const backgroundRemoverExecutor: NodeExecutor = {
  ...
  async execute({ node, inputs }) {
    ...
    const { rgbaImage, rgbaUrl, rgbaS3Key } = await matteImageForLoop({
      image,
