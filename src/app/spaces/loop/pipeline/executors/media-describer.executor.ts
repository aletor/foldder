 ...
 * Permite encadenar "Image Creation → Describer → …" dentro de una tubería de Loop.
 */

import { describeImageForLoop } from "../transports/loop-describe";
import {
...
export const mediaDescriberExecutor: NodeExecutor = {
  ...
  async execute({ inputs }) {
    ...
    const text = await describeImageForLoop({ url: img.url, s3Key: img.s3Key });
