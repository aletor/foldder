/**
 * Loop como orquestador de SUBGRAFO — capa de grafo pura (F0).
 *
 * Loop deja de envolver "un nodo" y pasa a envolver la tubería transitiva de
 ...
 * `promptInput`, `painter`…) y el propio Loop. Otras fuentes externas con lógica propia (p. ej. Brain)
...
/** Handle de entrada de Loop por el que llega la PLANTILLA / sink de la tubería. */
export const LOOP_PIPELINE_INPUT_HANDLE = "template" as const;
/** Handle de entrada de Loop por el que llega el Dataset (iterador, frontera). */
export const LOOP_DATASET_INPUT_HANDLE = "dataset" as const;
/** Tipo de nodo Dataset (frontera del recorrido). */
export const DATASET_NODE_TYPE = "dataset" as const;
/** Tipo de nodo Loop (para prohibir Loop anidado en v1). */
...
/**
 * IDs de los nodos que alimentan el handle de PLANTILLA de Loop (candidatos a sink).
 * En una configuración válida hay exactamente uno (ver validatePipeline).
 */
export function pipelineInputProducerIds(
  loopId: string,
  edges: readonly PipelineEdge[],
  pipelineInputHandle: string = LOOP_PIPELINE_INPUT_HANDLE,
): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  for (const e of edges) {
...
/**
 * El sink de la tubería: el único nodo cuyo output entra al handle de plantilla de Loop.
...
export function findPipelineSinkId(
  ...
  const producers = pipelineInputProducerIds(loopId, edges, pipelineInputHandle);
  return producers.length === 1 ? producers[0] : null;
}

/**
 * Todos los nodos cuyo output entra al handle de plantilla de Loop (un canal de salida
...
export function findPipelineSinkIds(
  ...
  return pipelineInputProducerIds(loopId, edges, pipelineInputHandle);
}

/**
 * IDs de todos los nodos de la tubería (recorrido hacia atrás desde el sink de Loop).
 * Excluye el propio Loop y los nodos frontera (Dataset). El orden es de descubrimiento,
...
export function discoverPipelineNodeIds<N extends PipelineNodeRef>(
  ...
  const pipelineInputHandle = opts.pipelineInputHandle ?? LOOP_PIPELINE_INPUT_HANDLE;
  ...
  const stack = pipelineInputProducerIds(loopId, edges, pipelineInputHandle).filter(
    (id) => id !== loopId && byId.has(id) && !isFrontier(id),
  );
...
export function discoverPipeline<N extends PipelineNodeRef>(
  ...
  const ids = new Set(discoverPipelineNodeIds(loopId, nodes, edges, opts));
 ...
 *  - Loop anidado dentro de la tubería: prohibido en v1.
 */
export function validatePipeline<N extends PipelineNodeRef>(args: {
  ...
  const { loopId, nodes, edges } = args;
  const pipelineInputHandle = args.pipelineInputHandle ?? LOOP_PIPELINE_INPUT_HANDLE;
  const errors: string[] = [];

  const sinkProducers = pipelineInputProducerIds(loopId, edges, pipelineInputHandle);
  if (sinkProducers.length === 0) {
    errors.push("La tubería no tiene plantilla: conecta un nodo al handle de Loop.");
  }
  ...
  const pipelineIds = discoverPipelineNodeIds(loopId, nodes, edges, { pipelineInputHandle });

  const byId = nodesById(nodes);
  const nested = pipelineIds.filter((id) => byId.get(id)?.type === LOOP_NODE_TYPE);
  if (nested.length > 0) {
    errors.push("Loop anidado dentro de la tubería no está soportado en v1.");
  }
...
/**
 * Análisis completo de una tubería de Loop: descubrimiento + orden + clasificación + validación.
 * `datasetBoundNodeIds` lo aporta el caller a partir de los bindings namespaced de Loop
...
export function analyzePipeline<N extends PipelineNodeRef>(args: {
  ...
  const pipelineNodeIds = discoverPipelineNodeIds(loopId, nodes, edges, { pipelineInputHandle });
  ...
  const validation = validatePipeline({ loopId, nodes, edges, pipelineInputHandle });
