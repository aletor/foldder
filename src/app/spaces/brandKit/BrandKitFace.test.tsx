...
import { BrandKitFace } from "./BrandKitFace";
import { crownedGenome, ghostGenome, proposedGenome } from "@/lib/brandkit/fixtures";
import { buildBookView } from "@/lib/brandkit/projection/book-view";

describe("BrandKitFace — cara estática (ghost · proposed · crowned)", () => {
  beforeEach(() => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({
  ...
  it("ghost: completitud 0 y rasgos en estado ghost", () => {
    ...
    const { container } = render(<BrandKitFace view={view} projectId="test" />);
  ...
  it("proposed: muestra propuestas, completitud > 0 y tap dispara onCrown", () => {
    ...
    const onCrown = vi.fn();
    const { container } = render(<BrandKitFace view={view} projectId="test" onCrown={onCrown} />);
  ...
  it("panel izquierdo: añadir url notifica hacia arriba", () => {
    ...
    render(<BrandKitFace view={view} projectId="test" onAddSource={onAddSource} />);
