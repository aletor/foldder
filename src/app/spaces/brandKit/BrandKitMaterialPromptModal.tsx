...
import type { ColorValue, LogoValue, TypographyValue } from "@/lib/brandkit/model/trait-values";
import { specimenFontStack } from "@/lib/brandkit/specimen/typography-specimen";
  ...
  if (prompt.traitId.startsWith("logo.") && value && typeof value === "object" && "imageUrl" in value) {
    ...
    return (
      <div className="mt-6 flex h-24 items-center justify-center border-t border-[var(--border)] pt-6">
        <BrandKitMediaImage src={logo.imageUrl} alt="" className="max-h-full max-w-full object-contain" eager />
      </div>
    );
  }
...
export function BrandKitMaterialPromptCard({
  prompt,
  onResolve,
  className,
}: {
  ...
  return (
    <div className={cx("border-t border-[var(--border)] pt-8", className)} role="group" aria-labelledby="brandKit-material-prompt-title">
      <p id="brandKit-material-prompt-title" className="text-sm leading-relaxed lowercase text-[var(--text-main)]">
        {prompt.headline}
      </p>
