 ...
 *
 * Estático por diseño: recibe una `BrandKitBookView` ya proyectada y notifica
...
import { BRAND_KIT_STYLE_GUIDE_EXPORT_MODE_LABELS } from "@/lib/brandkit/projection/style-guide-export-types";
import type { Genome } from "@/lib/brandkit/model/trait";
import { isVoiceProcessNoise } from "@/lib/brandkit/ingest/consolidated-registry";
import {
  SPECIMEN_SAMPLE_TEXT,
  typographyWeightCss,
} from "@/lib/brandkit/specimen/typography-specimen";
import type { BrandKitIngestFeedbackState } from "@/lib/brandkit/ingest/feedback-state";
import type { TraitId } from "@/lib/brandkit/model/trait-ids";
import type { ImageAxes, ImageDnaValue, VisualDnaFields } from "@/lib/brandkit/model/trait-values";
import type {
  ClaimValue,
  ColorValue,
  TaglineValue,
  ToneValue,
  TypographyValue,
} from "@/lib/brandkit/model/trait-values";
import type {
  FaceState,
  BrandKitBookView,
  MultiTraitSlot,
  TraitSlot,
} from "@/lib/brandkit/projection/book-view";
...
import { BrandKitIngestFeedback } from "./BrandKitIngestFeedback";
import { BrandKitPageVisionBadge, BrandKitSourcesPanel } from "./BrandKitPageVisionBadge";
import { BrandKitDepthPanel } from "./BrandKitDepthPanel";
import { BrandKitFaceProvider, useBrandKitFaceContext } from "./brand-kit-face-context";
...
export interface BrandKitFaceProps {
  ...
  ingestFeedback?: BrandKitIngestFeedbackState;
  onIngestRetry?: () => void;
  activePrompt?: import("@/lib/brandkit/ingest/material-prompt").MaterialPromptPayload | null;
  ...
  return (
    <>
      <button
        ...
        className="brandKit-depth-trigger"
      >
        evidencia
      </button>
      {open && ctx?.genome ? (
...
function DepthButton({ slot }: { slot: TraitSlot<unknown> }) {
  ...
  return (
    <div className="brandKit-depth-trigger-wrap">
...
function SpecimenBlock({
  ...
  label: string;
  onCrown?: BrandKitFaceProps["onCrown"];
  onUploadSpecimenFont?: BrandKitFaceProps["onUploadSpecimenFont"];
}) {
  const v = slot.value;
  useSpecimenFont(v);
...
function ColorBlock({ slot, onCrown }: { slot: TraitSlot<ColorValue>; onCrown?: BrandKitFaceProps["onCrown"] }) {
  const v = slot.value;
...
function PaletteSection({
  ...
  palette: BrandKitBookView["palette"];
  onCrown?: BrandKitFaceProps["onCrown"];
}) {
  return (
...
function VoiceSection({
  ...
  claimsForbidden,
  onCrown,
}: BrandKitBookView["voice"] & { onCrown?: BrandKitFaceProps["onCrown"] }) {
  return (
...
function VisualTerritoryCard({
  ...
  onCrown?: BrandKitFaceProps["onCrown"];
  onConfirmVisual?: BrandKitFaceProps["onConfirmVisual"];
}) {
  ...
  if (generated) {
    return (
      ...
      >
        <BrandKitMediaImage src={generated} alt={category} className="aspect-[4/3] w-full object-cover" eager />
...
function VisualUniverseSection({
  ...
  visualUniverse: BrandKitBookView["visualUniverse"];
  onCrown?: BrandKitFaceProps["onCrown"];
  onConfirmVisual?: BrandKitFaceProps["onConfirmVisual"];
}) {
...
function LeftPanel({
  ...
  projectId: string;
  view: BrandKitBookView;
  onAddSource?: BrandKitFaceProps["onAddSource"];
  onDrop?: BrandKitFaceProps["onDrop"];
  onDownload?: BrandKitFaceProps["onDownload"];
  ingestFeedback?: BrandKitIngestFeedbackState;
  onIngestRetry?: BrandKitFaceProps["onIngestRetry"];
  activePrompt?: BrandKitFaceProps["activePrompt"];
  onResolvePrompt?: BrandKitFaceProps["onResolvePrompt"];
}) {
  ...
  return (
    <aside className="flex h-full w-[20%] min-w-[280px] max-w-[420px] flex-col overflow-hidden border-r border-[var(--border)] bg-[var(--surface)]">
      <div className={cx("min-h-0 flex-1 overflow-y-auto overscroll-contain", G.panel, "flex flex-col gap-8 pb-6")}>
        ...
        <BrandKitSourcesPanel sources={view.sources} />
...
export function BrandKitFace({
  ...
  return (
    <BrandKitFaceProvider value={slotProps}>
    ...
    </BrandKitFaceProvider>
  );
}

export default BrandKitFace;
