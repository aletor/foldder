...
import type {
  ...
  LogoProposal,
} from "@/lib/brandkit/logo-intake/types";
import type { ValidateLogoIntakeResult } from "@/lib/brandkit/logo-intake/service";
import { crownedCandidates, type Genome } from "@/lib/brandkit/model/trait";
import type { LogoValue } from "@/lib/brandkit/model/trait-values";
import type { TraitId } from "@/lib/brandkit/model/trait-ids";
import { formatLogoIntakeProvenance } from "@/lib/brandkit/logo-intake/genome-bridge";
import { resolveLogoDisplayUrl } from "@/lib/brandkit/projection/logo-display-url";
...
import { humanizeLogoIntakeError } from "@/lib/brandkit/logo-intake/error-copy";
...
>(function LogoIntakePanel({ projectId, genome, onGenomeChange }, ref) {
  ...
  const faceCtx = useBrandKitFaceContext();

  const genomeRef = useRef(genome);
  genomeRef.current = genome;

  const loadProposal = useCallback(async () => {
    const res = await fetch(`/api/brandKit/logo-intake/proposal?projectId=${encodeURIComponent(projectId)}`);
    if (res.status === 404) {
      setProposal(null);
      setSelectedId(null);
      return;
  ...
  const runUndoValidate = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/brandKit/logo-intake/undo-validate", {
        method: "POST",
  ...
  const loadState = useCallback(async () => {
    const res = await fetch(`/api/brandKit/logo-intake/analyze?projectId=${encodeURIComponent(projectId)}`);
    ...
    async (candidate: LogoCandidate) => {
      ...
      try {
        const res = await fetch(
          `/api/brandKit/logo-intake/candidate-preview?projectId=${encodeURIComponent(projectId)}&candidateId=${encodeURIComponent(candidate.id)}`,
        );
  ...
  const validateSelected = useCallback(
    async (kind: "accept_best" | "accept_alternative") => {
      ...
      setError(null);
      try {
        const res = await fetch("/api/brandKit/logo-intake/validate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            projectId,
  ...
  const validateManual = useCallback(
    async (file: File) => {
      ...
      try {
        const res = await fetch("/api/brandKit/logo-intake/validate", { method: "POST", body: form });
        const data = await readJson<ValidateLogoIntakeResult>(res);
        await applyValidateResult(data);
      } catch (err) {
  ...
  return (
    <section className={cx("relative flex min-h-[70vh] w-full flex-col items-center justify-center gap-10 self-stretch bg-[#1A1B1E] px-8 py-16 text-white", G.section)}>
      <div className="brandKit-depth-trigger-wrap brandKit-depth-trigger-wrap--dark">
    ...
    <div className="flex w-full flex-col items-center gap-4">
      <div className={cx("grid w-full gap-px bg-white/10", large ? "grid-cols-1 md:grid-cols-2" : "grid-cols-2")}>
        <div className={cx("flex items-center justify-center bg-white p-8", large && "min-h-[220px]")}>
          <BrandKitMediaImage src={src} alt="logo propuesto" className="max-h-[28vh] max-w-full object-contain" eager />
        </div>
        <div className={cx("flex items-center justify-center bg-[#0d0d0f] p-8", large && "min-h-[220px]")}>
          <BrandKitMediaImage src={src} alt="" aria-hidden className="max-h-[28vh] max-w-full object-contain" eager />
        </div>
      </div>
...
function ValidatedView({
  ...
  const candidate = {
    ...
    assetUrl:
      genomeLogoUrl ??
      `/api/brandKit/logo-intake/asset?projectId=${encodeURIComponent(projectId)}&format=png`,
  };
