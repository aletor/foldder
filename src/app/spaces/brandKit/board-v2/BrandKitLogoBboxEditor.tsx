...
import { extractPreviewDataUrl, loadPageCanvas, trimBBoxOnPage } from "@/lib/brandkit/logo-intake/bbox-editor-client";
import type { LogoValue } from "@/lib/brandkit/brand-kit-types";
import { isValidBboxPage, logoSourceBboxToPageTuple } from "@/lib/brandkit/brand-kit-logo-bbox";
...
export function BrandKitLogoBboxEditor({
  logo,
  onClose,
  onSaved,
}: {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/spaces/brandKit/logo-adjust/page?${params.toString()}`)
      .then(async (res) => {
  ...
  const save = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/spaces/brandKit/logo-adjust/crop", {
        method: "POST",
  ...
  const pageLabel = brandKitLocaleEs.logoPageSignal(currentPageNumber, totalPages);

  return (
    <div className="brandKit-v2-logo-adjust" role="dialog" aria-modal="true">
      <div className="brandKit-v2-logo-adjust__header">
        <div>
          <p className="brandKit-v2-logo-adjust__title">{brandKitLocaleEs.adjustLogoArea}</p>
          {pageLabel ? <p className="brandKit-v2-muted">{logo.sourceDocName ?? ""} · {pageLabel}</p> : null}
          {showPagePicker ? (
            <label className="brandKit-v2-logo-adjust__page-picker">
              <span>{brandKitLocaleEs.logoEditorPageLabel}</span>
              <select
        ...
        <button type="button" className="brandKit-v2-icon-btn" onClick={onClose} aria-label="Cerrar">
          <X size={18} />
        </button>
      </div>

      {loading ? <p className="brandKit-v2-muted">Cargando página…</p> : null}
      {error ? <p className="brandKit-v2-error">{error}</p> : null}

      {!loading && page ? (
        <div className="brandKit-v2-logo-adjust__body">
          <div className="brandKit-v2-logo-adjust__stage">
            <div
              className="brandKit-v2-logo-adjust__canvas"
            ...
            >
              <BrandKitMediaImage
                ...
                alt=""
                className="brandKit-v2-logo-adjust__page"
                draggable={false}
                width={page.width}
                height={page.height}
                eager
              />
              <div className="brandKit-v2-logo-adjust__overlay">
                <div
                  className="brandKit-v2-logo-adjust__bbox"
                  style={bboxCss}
                ...
                >
                  {HANDLES.map((handle) => (
                    <span
                      key={handle}
                      role="presentation"
                      className="brandKit-v2-logo-adjust__handle"
          ...
          <aside className="brandKit-v2-logo-adjust__preview">
            <p className="brandKit-v2-logo-adjust__preview-label">{brandKitLocaleEs.logoCropPreview}</p>
            <div className="brandKit-v2-logo-adjust__preview-frame">
              {previewUrl ? (
                <BrandKitMediaImage src={previewUrl} alt="" className="brandKit-v2-logo-adjust__preview-img" eager />
              ) : (
