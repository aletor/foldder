...
import type { PageVisionPassSourceMeta } from "@/lib/brandkit/ingest/page-vision-pass-meta";
import {
  pageVisionPassBadgeLabel,
  pageVisionPassSkipDetail,
} from "@/lib/brandkit/ingest/page-vision-pass-meta";
import type { BrandKitSourceView } from "@/lib/brandkit/projection/book-view";
import { G, cx } from "./face-utils";

export function BrandKitPageVisionBadge({
  meta,
  className,
  title,
}: {
...
export function BrandKitSourcesPanel({ sources }: { sources: BrandKitSourceView[] }) {
  if (!sources.length) return null;
  return (
    <section className="border-t border-[var(--border)] pt-8" data-testid="brandKit-sources-panel">
        ...
        {sources.map((source) => (
          <li key={source.id} className={cx(G.listRow, "flex flex-col gap-2")}>
            <div className="flex items-baseline justify-between gap-4">
              ...
              {source.kind === "pdf" ? <BrandKitPageVisionBadge meta={source.pageVisionPass} /> : null}
            </div>
