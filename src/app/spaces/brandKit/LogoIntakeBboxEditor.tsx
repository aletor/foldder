...
import {
  ...
  loadPageCanvas,
  trimBBoxOnPage,
} from "@/lib/brandkit/logo-intake/bbox-editor-client";
...
export function LogoIntakeBboxEditor({
  ...
  useEffect(() => {
    ...
    setError(null);
    void fetch(
      `/api/brandKit/logo-intake/edit-page?projectId=${encodeURIComponent(projectId)}&candidateId=${encodeURIComponent(candidateId)}`,
    )
      .then(async (res) => {
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        ...
        <aside className="flex w-full shrink-0 flex-col gap-4 border-t border-white/10 bg-[#121214] p-6 lg:w-80 lg:border-t-0 lg:border-l">
          ...
          <div className="flex min-h-[120px] items-center justify-center border border-white/15 bg-white p-4">
            {previewUrl ? (
              <BrandKitMediaImage src={previewUrl} alt="preview recorte" className="max-h-[28vh] max-w-full object-contain" eager />
            ) : (
