...
import { resolveMaterialPrompt } from "@/lib/brandkit/ingest/material-prompt";
...
import { streamCombinedMaterialDrop, streamBrandKitIngest } from "./brand-kit-ingest-client";

export function useBrandKitIngest(options: {
  genome: Genome;
  ...
  const [feedback, setFeedback] = useState<BrandKitIngestFeedbackState>(() => idleIngestFeedback(options.genome));
  ...
  const runStream = useCallback(
    async (input: { files?: FileList | File[]; url?: string }) => {
      ...
      setPromptQueue([]);
      try {
        const handleEvent = (event: BrandKitIngestStreamEvent) => {
        ...
        const { genome } = await streamBrandKitIngest({
          files: input.files,
          url: input.url,
          genome: genomeRef.current,
          onEvent: handleEvent,
