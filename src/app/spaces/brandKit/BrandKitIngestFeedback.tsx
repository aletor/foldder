...
import {
  ...
  type BrandKitIngestFeedbackState,
  type IngestTimelineStep,
  type SectionRowState,
} from "@/lib/brandkit/ingest/feedback-state";
import { copySourceUnreadable } from "@/lib/brandkit/ingest/feedback-copy";
import { BrandKitMaterialPromptCard } from "./BrandKitMaterialPromptModal";
import { G, cx } from "./face-utils";
import { BrandKitMediaImage } from "./BrandKitMediaImage";
import { useMicroFade } from "./use-brand-kit-ingest";
...
function SectionPreview({ preview }: { preview: SectionRowState["preview"] }) {
  ...
  if (p.kind === "logo") {
    return (
      <BrandKitMediaImage src={p.imageUrl} alt="" className="h-5 w-auto max-w-[64px] object-contain opacity-80" eager />
    );
  }
...
function ConsolidatedRow({ id, row }: { id: (typeof BRAND_KIT_INGEST_SECTION_ORDER)[number]; row: ConsolidatedRowState }) {
...
function ActivitySectionRow({
  ...
  id: (typeof BRAND_KIT_INGEST_SECTION_ORDER)[number];
  row: SectionRowState;
  onRetry?: () => void;
}) {
  const label = sectionLabel(id);
...
export function BrandKitIngestFeedback({
  feedback,
  onRetry,
  activePrompt,
  onResolvePrompt,
}: {
  feedback: BrandKitIngestFeedbackState;
  onRetry?: () => void;
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {showConsolidated ? (
        <section aria-label="tu brandKit consolidado" className="border-t border-[var(--border)] pt-8">
        ...
        <section role="status" aria-live="polite" className="border-t border-[var(--border)] pt-8">
          {activity.statusLine ? (
            ...
            <p className={cx(G.label, "mb-6")}>
              {activity.incremental ? "material nuevo" : `consolidando tu brandKit · ${activity.receiveCount} documentos`}
            </p>
          ) : null}
          ...
          {activePrompt && onResolvePrompt ? (
            <BrandKitMaterialPromptCard prompt={activePrompt} onResolve={onResolvePrompt} className="mt-8" />
          ) : null}
          {micro ? (
...
export function BrandKitIngestStatusLine({ line }: { line: string | null }) {
  if (!line) return null;
  return (
    <p className="mb-6 flex items-center gap-3 text-sm lowercase text-[var(--text-muted)]">
      <FineSpinner />
