/**
 * Nested Space portal ↔ Populate — resolución de plantilla interna y reglas de cableado.
 */
...
export function resolveSpacePortalInnerTemplate(
  ...
  if (typeof cachedType === "string" && POPULATE_PIPELINE_EXECUTABLE_TYPES.has(cachedType)) {
    const sourceHandle =
  ...
  const executable = sinks.find((s) =>
    POPULATE_PIPELINE_EXECUTABLE_TYPES.has(String(s.node.type ?? "")),
  );
...
/** Populate deposita resultados en un Space: `out|media_list → in` (ambas direcciones de cableado). */
export function isPopulateSpacePortalConnection(
  ...
  if (!populate || !space) return false;

  const populateIsSource =
    connection.source != null
      ? connection.source === populate.id
      : sourceNode.id === populate.id;
  const populateHandle = populateIsSource ? connection.sourceHandle : connection.targetHandle;
  const spaceHandle = populateIsSource ? connection.targetHandle : connection.sourceHandle;

  if (spaceHandle !== "in") return false;
  return populateHandle === "out" || populateHandle === "media_list";
}
...
export function expandSpacePortalTemplateForPipeline(
  ...
  for (const edge of edges) {
    if (edge.targetHandle !== POPULATE_SINK_TARGET_HANDLE) continue;
