...
describe("populate-space-portal", () => {
  it("finds a space node connected to populate", () => {
    ...
    expect(findPopulateSpacePortalNode("populate_1", [populate, space], edges)?.id).toBe("space_portal");
  });

  it("places portal to the right of populate", () => {
    const pos = computeSpacePortalPositionBeside(populate);
    expect(pos.x).toBeGreaterThan(populate.position.x);
  });
  ...
  it("reuses spaceId when portal is connected", () => {
    ...
    const resolved = resolvePopulateCommitSpaceId("populate_1", portal);
  ...
  it("creates new ids when no portal is connected", () => {
    const resolved = resolvePopulateCommitSpaceId("populate_1", null);
  ...
  it("builds populate out → space in edge", () => {
    const edge = buildPopulateToSpaceEdge("populate_1", "space_portal");
