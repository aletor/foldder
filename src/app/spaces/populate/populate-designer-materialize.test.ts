...
describe("freezeDesignerPagesForRow (Modo 2 · huecos pendientes)", () => {
  it("resuelve el hueco pendiente con la columna mapeada por Populate", () => {
    const map = {
...
describe("buildDesignerGeneratedSubgraph", () => {
  it("crea un nodo designer por fila, sin edges", () => {
    ...
    expect(sub.nodes[0]!.id).toBe("pop_pop1_r0_designer");
    expect(sub.nodes[0]!.type).toBe("designer");
    expect((sub.nodes[1]!.data as { _populateRowCardId?: string })._populateRowCardId).toBe("c2");
  });
});
