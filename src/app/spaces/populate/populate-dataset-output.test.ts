...
describe("populate-dataset-output", () => {
  it("creates column and writes rows", async () => {
    const { applyPopulateResultsToDataset } = await import("./populate-dataset-output");
    const dataset = {
      id: "ds1",
      name: "Test",
      scope: "local",
  ...
  it("multi-canal: cada canal escribe en su propia columna (acumulando)", async () => {
    const { applyPopulateChannelsToDataset } = await import("./populate-dataset-output");
    const dataset = {
      id: "ds1",
      name: "Test",
      scope: "local",
