/**
 * Populate Studio — slots dinámicos, resumen legible y bloqueadores de ejecución.
 */
...
import type { PopulateBindings } from "./populate-types";

export type PopulateStudioSlotKind = "prompt" | "token" | "ref";

export interface PopulateStudioSlot {
  id: string;
  kind: PopulateStudioSlotKind;
  label: string;
...
export interface PopulateStudioSummary {
...
export function estimatePopulateImageCostUsd(model: PopulateTemplateModel): number {
...
export function buildPopulateStudioSlots(args: {
  promptText: string;
  ...
  const slots: PopulateStudioSlot[] = [
    {
      id: "prompt",
      kind: "prompt",
      label: promptLabel,
...
export function buildPopulateStudioSummary(args: {
  templateLabel: string;
  listName: string;
  rowCount: number;
  promptText: string;
  ...
  const costPerImageUsd = estimatePopulateImageCostUsd(model);
  const costTotalUsd = costPerImageUsd * Math.max(0, rowCount);

  const blockers: string[] = [];
  if (!datasetConnected) blockers.push("Conecta un Dataset al nodo Populate.");
...
export function buildPopulateCompactSummary(args: {
  listName: string;
  rowCount: number;
