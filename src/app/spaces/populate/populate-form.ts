/**
 * Populate — modo FORMULARIO (relleno manual, un resultado al instante).
 *
...
export interface PopulateFormImageOption {
  rowIndex: number;
  label: string;
  url: string;
}

export interface PopulateFormImageField {
  inputId: string;
  label: string;
  listId: string;
  fieldId: string;
  options: PopulateFormImageOption[];
}

export interface PopulateFormRow {
  rowIndex: number;
  label: string;
}

export interface PopulateFormModel {
  textFields: PopulateFormTextField[];
  imageFields: PopulateFormImageField[];
...
export function derivePopulateForm(args: {
  ...
  const empty: PopulateFormModel = { textFields: [], imageFields: [], rows: [], empty: true };
  ...
  const textFields: PopulateFormTextField[] = [];
  ...
  const imageFields: PopulateFormImageField[] = [];
  for (const slot of imageInputs) {
    ...
    const options: PopulateFormImageOption[] = [];
    for (let i = 0; i < rowCount; i += 1) {
