...
import type { DesignerFormImageOption } from "./populate-designer-form";
...
export type DesignerFormImagePickerProps = {
  ...
  /** Estilos del panel Populate Studio (por defecto) o formulario público. */
...
export function DesignerFormImagePicker({
  ...
  if (options.length === 0) {
    return (
      <div className={isPublic ? "flex flex-col gap-1" : "populate-form-panel__field"}>
        <span
          className={
            isPublic
              ? "text-[10px] font-extrabold uppercase tracking-wider text-[#fd52eb]/85"
              : "populate-form-panel__label"
          }
        >
          {label}
        </span>
        <span
          className={isPublic ? "text-sm italic text-white/45" : "populate-form-panel__constant"}
          title={emptyHint}
        >
  ...
  const searchClass = isPublic
    ...
    : "designer-form-image-picker__search nodrag populate-form-panel__input";
  ...
  return (
    <div
      className={
        isPublic ? "flex flex-col gap-2" : "populate-form-panel__field designer-form-image-picker"
      }
    >
      <span
        className={
