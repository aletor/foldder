/**
 * Populate como orquestador de subgrafo — contrato de ejecución por nodo (F1).
 *
...
/** Variable bindeable a una columna del Dataset (vista por el panel de Populate). */
export interface BindableVar {
  /** Clave local del nodo (p. ej. "prompt", "image2"). Populate la namespacea como `<nodeId>.<key>`. */
  key: string;
  label: string;
  type: CreativeInputKind;
