...
const describeImageForPopulate = vi.fn();
vi.mock("../transports/populate-describe", () => ({
  describeImageForPopulate: (args: unknown) => describeImageForPopulate(args),
}));
...
beforeEach(() => {
  describeImageForPopulate.mockReset();
  describeImageForPopulate.mockResolvedValue("una foto de producto sobre fondo neutro");
});
...
describe("mediaDescriberExecutor", () => {
  it("describe la imagen de entrada (cualquier handle de imagen) y devuelve texto", async () => {
    ...
    expect(describeImageForPopulate).toHaveBeenCalledWith({ url: "https://img/in.png", s3Key: "k1" });
  });
  ...
  it("falla con mensaje claro si no hay imagen de entrada", async () => {
    ...
    expect(describeImageForPopulate).not.toHaveBeenCalled();
  });
});
