/**
 * Populate como orquestador de subgrafo — motor de ejecución (F2).
 *
 ...
 * resolución de inputs/overrides se inyecta vía `buildInputs`, que el adaptador de Populate cablea
  ...
  if (missing.length > 0) {
    throw new Error(
      `La tubería tiene nodos sin executor registrado: ${missing.join(", ")}. ` +
        "Conéctala solo a nodos soportados por Populate.",
    );
  }
