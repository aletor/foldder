/**
 * Populate — escribir los slides rasterizados de un Designer congelado de vuelta al Dataset.
 *
 * Modelo: M columnas (una por slide) × N filas. Cada fila recibe sus propios slides. Las columnas
 * se identifican por `slideKey` (estable) + `populateGroupId` (el grupo de este Populate-Designer),
...
import { slugDatasetColumnKey } from "./populate-dataset-output";
...
export interface DesignerDatasetOutputSettings {
  enabled: boolean;
  /** Id estable del grupo de columnas (un grupo por Populate-Designer). */
  groupId: string;
...
export function applyDesignerSlidesToDataset(args: {
  ...
  canonical.forEach((slide, index) => {
    ...
    const existingIdx = schema.findIndex(
      (f) => f.populateGroupId === settings.groupId && f.populateSlideKey === slide.slideKey,
    );
    if (existingIdx >= 0) {
    ...
    const field: FieldDef = {
      ...base,
      populateGroupId: settings.groupId,
      populateSlideKey: slide.slideKey,
      orphaned: false,
    };
    schema = [...schema, field];
  ...
  schema = schema.map((f) => {
    if (f.populateGroupId === settings.groupId && f.populateSlideKey && !seen.has(f.populateSlideKey)) {
...
/** Genera un id de grupo estable para un Populate-Designer (una vez por nodo). */
export function makePopulateDesignerGroupId(populateNodeId: string): string {
  return `pdg_${populateNodeId}`;
}
