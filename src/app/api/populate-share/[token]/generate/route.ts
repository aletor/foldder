...
} from "@/app/spaces/populate/populate-form";
...
import {
  generatePopulateShareImage,
  GeminiGenerateError,
  OpenAiGenerateError,
} from "@/lib/populate-share-generate";
...
export async function POST(req: Request, context: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    walletCharge = await reserveApiWalletCharge({
      ...
      serviceId,
      provider,
      route: "/api/populate-share/generate",
      maxCostMicros: reserveUsdToMicros(estimatedCostUsd, { multiplier: 1.15 }),
      metadata: { populateShareToken: token, populateNodeId: row.populateNodeId },
    });

    const result = await generatePopulateShareImage({
      prompt,
    ...
    console.error("[populate-share/generate] failed:", error);
