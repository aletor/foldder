...
import {
  createLoopShare,
  findLoopShareByToken,
  listLoopShares,
  updateLoopShare,
} from "@/lib/loop-share-db";
import type { LoopSharePayload, LoopShareRecord } from "@/lib/loop-share-types";
import { DEFAULT_LOOP_SHARE_OPTIONS } from "@/lib/loop-share-types";
...
function isValidPayload(payload: unknown): payload is LoopSharePayload {
  if (!payload || typeof payload !== "object") return false;
  const p = payload as LoopSharePayload;
  const baseOk =
...
function payloadHasContent(p: LoopSharePayload): boolean {
  if (p.designer != null) return p.designer.formFields.length > 0;
  return !p.formModel.empty;
}

/** GET ?shareKey= — lista enlaces de un nodo Loop. */
export async function GET(req: Request) {
  try {
    ...
    const list = await listLoopShares(shareKey);
    const safe = list.map((r) => ({
      id: r.id,
      token: r.token,
      shareKey: r.shareKey,
      loopNodeId: r.loopNodeId,
      name: r.name,
      slug: r.slug,
      visits: r.visits,
    ...
    console.error("[loop-share][GET] failed:", error);
...
type PostBody = {
  ...
  options?: Partial<LoopShareRecord["options"]>;
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.loopNodeId || typeof body.loopNodeId !== "string") {
      return NextResponse.json({ error: "loopNodeId required" }, { status: 400 });
    }
    ...
    if (existingToken) {
      const existing = await findLoopShareByToken(existingToken);
      if (!existing) {
      ...
      const updated = await updateLoopShare(existingToken, {
        name: body.name.trim(),
        options,
    ...
    const record: LoopShareRecord = {
      id: uuidv4(),
      token,
      shareKey: body.shareKey,
      loopNodeId: body.loopNodeId,
    ...
    console.error("[loop-share][POST] failed:", e);
