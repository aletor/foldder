...
import { refineCrownedRasterLogo } from "@/lib/brandkit/ingest/logo-crown-refine";
import { brandKitOperationId } from "@/lib/brandkit/ingest/paid-operations";
import { resolveHiResLogoRasterForVectorize } from "@/lib/brandkit/ingest/vectorize-hires-raster";
import type { LogoVectorSourceRef } from "@/lib/brandkit/model/evidence";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.cachedVectorUrl?.trim()) {
      console.info(
        `[vectorize] called: reason=brand_kit_cached_body logoSignature=${logoSignature} cached=true`,
      );
    ...
    const svg = await vectorizeRasterBuffer({
      ...
      mode: "production",
      audit: {
        reason: hiRes.source === "hi_res_pdf_crop" ? "brand_kit_crown_hi_res" : "brand_kit_crown_raster",
        logoSignature,
        cached: false,
      },
    });
    ...
    const vectorKey = buildUserAssetObjectKey({
      userEmail: auth.user.email,
      folder: `brandKit/logos/vector/${sig}`,
    ...
    console.error("[brandKit/logo/vectorize]", error);
