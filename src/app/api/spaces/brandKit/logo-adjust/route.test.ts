...
vi.mock("@/lib/brandkit/ingest/brand-kit-source-pdf-store", () => ({
  loadBrandKitSourceForLogoAdjust: vi.fn(async () => ({
    buffer: mockPdfBuffer,
    kind: "pdf" as const,
  })),
}));

vi.mock("@/lib/brandkit/ingest/brand-kit-logo-adjust-page", () => ({
...
vi.mock("@/lib/brandkit/brand-kit-logo-crop-server", () => ({
...
vi.mock("@/lib/brandkit/ingest/upload-brand-kit-file", () => ({
...
describe("/api/spaces/brandKit/logo-adjust", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("GET page returns raster + bbox for valid params", async () => {
    const url = new URL("http://localhost/api/spaces/brandKit/logo-adjust/page");
  ...
  it("GET page rejects missing fields", async () => {
    const res = await getPage(new NextRequest("http://localhost/api/spaces/brandKit/logo-adjust/page"));
  ...
  it("POST crop returns adjusted logo", async () => {
    const res = await postCrop(
      new Request("http://localhost/api/spaces/brandKit/logo-adjust/crop", {
        method: "POST",
