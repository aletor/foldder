...
} from "@/lib/genoma/projection/style-guide-vectorize-gate";
import { verifyStyleGuidePdfFonts } from "@/lib/genoma/projection/style-guide-font-gate";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const doc = await renderGenomaStyleGuide(genome, {
    ...
    try {
      ...
      console.warn("[genoma/style-guide/pdf] font verification skipped", fontErr);
    }
    if (!fontGate.allowed) {
      return NextResponse.json(
        {
    ...
    const filename = genomaStyleGuideFilename(body.projectName, doc.generatedAt);
    ...
    console.error("[genoma/style-guide/pdf]", error);
    return NextResponse.json(
      {
