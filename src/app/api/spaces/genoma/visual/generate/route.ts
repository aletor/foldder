...
import { reserveGenomaVisualGenerateCharge } from "@/lib/genoma/ingest/genoma-visual-wallet";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    walletCharge = await reserveGenomaVisualGenerateCharge({
      req,
    ...
    const result = await geminiImageGenerate(
      ...
      undefined,
      { usageUserEmail: auth.user.email, usageRoute: "/api/spaces/genoma/visual/generate" },
    );
