...
import { estimateGenomaGalleryGenerateCostUsd } from "@/lib/genoma/genoma-gallery-cost";
import {
  galleryGenerateActualCostUsd,
  GENOMA_GALLERY_IMAGE_COUNT,
  IMAGE_MODEL,
  runGenomaGalleryGenerate,
} from "@/lib/genoma/run-gallery-generate";
...
function parseGenomaSnapshot(body: unknown): GenomaDocument | null {
  if (!body || typeof body !== "object") return null;
  const genoma = (body as { genoma?: GenomaDocument }).genoma;
  if (!genoma?.slots) return null;
  return genoma;
}

function estimateGalleryGenerateReserveUsd(): number {
  return Math.round(estimateGenomaGalleryGenerateCostUsd() * 1_000_000) / 1_000_000;
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const genoma = parseGenomaSnapshot(body);
    if (!genoma) {
      return Response.json({ error: "genoma snapshot required" }, { status: 400 });
    }

    const gallerySlot = genoma.slots.gallery;
    ...
    walletCharge = await reserveApiWalletCharge({
      ...
      metadata: { count: GENOMA_GALLERY_IMAGE_COUNT, model: IMAGE_MODEL },
    });
    ...
    if (acceptStream) {
      ...
      const stream = new ReadableStream<Uint8Array>({
        async start(controller) {
          ...
          try {
            for await (const event of runGenomaGalleryGenerate({
              genoma,
              stylePromptVersion,
              userEmail: authState.user.email,
            })) {
    ...
    return Response.json({
      ...
      stylePrompt,
      addedCount,
      partial: addedCount < GENOMA_GALLERY_IMAGE_COUNT,
    });
  } catch (error) {
    if (releaseWalletOnError) await releaseApiWalletChargeOnError(walletCharge, error);
