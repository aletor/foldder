...
import { createEmptyGenoma, mergeGenomaDocument, normalizeGenomaDocument } from "./genoma-defaults";
import { hasLegacyGenomaSlots, LEGACY_SLOT_KEYS, migrateGenomaDocument } from "./genoma-migrate-v2";
import type { GenomaDocument, Provenance, SlotState } from "./genoma-types";
...
describe("genoma migrate v2", () => {
  it("detects legacy slots in raw documents", () => {
    const legacy = createAlimaLegacyFixture();
    expect(hasLegacyGenomaSlots(legacy)).toBe(true);
    expect(hasLegacyGenomaSlots(createEmptyGenoma())).toBe(false);
  });

  it("migrates essence from oneliner + values with AND lock merge", () => {
    const legacy = createAlimaLegacyFixture();
    const migrated = migrateGenomaDocument(createEmptyGenoma(), legacySlots(legacy));
  ...
  it("locks essence only when all contributing legacy slots with content were locked", () => {
    ...
    const migrated = migrateGenomaDocument(createEmptyGenoma(), slots);
  ...
  it("oneliner locked alone locks essence when values is empty", () => {
    ...
    const slots = legacySlots(legacy);
    slots.oneliner = { ...slots.oneliner!, locked: true };
    slots.values = { ...createEmptyGenoma().slots.voice, id: "values", status: "empty", candidates: [], confidence: 0, locked: false, history: [], updatedAt: NOW };
    const migrated = migrateGenomaDocument(createEmptyGenoma(), slots);
    expect(migrated.slots.essence?.locked).toBe(true);
  });
  ...
  it("never propagates prohibitions lock to voice or visualWorld", () => {
    const legacy = createAlimaLegacyFixture();
    const migrated = migrateGenomaDocument(mergeGenomaDocument(legacy), legacySlots(legacy));
  ...
  it("normalize persists v2-only document without legacy slot keys", () => {
    const legacy = createAlimaLegacyFixture();
    const normalized = normalizeGenomaDocument(legacy) as GenomaDocument & { slots: Record<string, unknown> };
  ...
  it("preserves user edits after migration roundtrip", () => {
    const legacy = createAlimaLegacyFixture();
    let doc = normalizeGenomaDocument(legacy);
    ...
    const reloaded = normalizeGenomaDocument(doc);
  ...
  it("compiles migrated document with beliefs and headline", async () => {
    const legacy = createAlimaLegacyFixture();
    const normalized = normalizeGenomaDocument(legacy);
    const compiled = await compileGenoma(normalized);
