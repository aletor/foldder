/**
 * Extractor de logo (§3.2) — capa Genoma sobre el pipeline de páginas renderizadas.
 *
...
import {
  detectGenomaLogosFromPdfBuffer,
...
export type GenomaLogoCandidate = {
  buffer: Buffer;
...
export type GenomaLogoExtraction = {
  logos: GenomaLogoCandidate[];
  primaryLogos: GenomaLogoCandidate[];
  secondaryLogos: GenomaLogoCandidate[];
...
): Promise<GenomaLogoExtraction> {
  const { logos: raw, ambiguousPrimary } = await detectGenomaLogosFromPdfBuffer(buffer, options);
  const logos: GenomaLogoCandidate[] = [];
