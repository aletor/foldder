...
import { genomaOperationId, textSampleSignature } from "../ingest/paid-operations";
import type { VoiceExtraction, PdfTextLine } from "./voice";

const GENOMA_VOICE_SERVICE_ROUTE = "/lib/genoma/extractors/voice-llm";
...
export async function refineVoiceWithLlm(
  ...
  const operationId = genomaOperationId("voice", cacheKey);
  ...
  try {
    if (userEmail) {
      walletCharge = await reserveApiWalletCharge({
        ...
        provider: "gemini",
        route: GENOMA_VOICE_SERVICE_ROUTE,
        maxCostMicros: reserveUsdToMicros(0.02, { multiplier: 1.25 }),
        operationId,
        metadata: { feature: "genoma-voice-refine", sampleSignature: cacheKey },
      });
    }
