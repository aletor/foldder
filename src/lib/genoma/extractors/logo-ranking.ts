...
export type RankedGenomaLogoHarvest = ScoredGenomaLogoHarvest & { slot: "primary" | "secondary" };

function applySimpleSolidBrandPenalty(entry: ScoredGenomaLogoHarvest): ScoredGenomaLogoHarvest {
...
function isViableLogo(entry: ScoredGenomaLogoHarvest): boolean {
...
function isPhotoLikePrimary(entry: ScoredGenomaLogoHarvest): boolean {
...
function assignPrimarySlot(entry: ScoredGenomaLogoHarvest, rank: number): "primary" | "secondary" | "discard" {
...
/** Variantes de polaridad sintetizadas no cuentan como candidatos distintos. */
function isPrimaryContender(entry: ScoredGenomaLogoHarvest): boolean {
  if (!isViableLogo(entry)) return false;
  if (entry.evidenceDetail?.includes("sintetizado")) return false;
  return true;
}

export function detectAmbiguousLogoPrimaries(entries: ScoredGenomaLogoHarvest[]): boolean {
...
export function finalizeLogoHarvestRanking(entries: ScoredGenomaLogoHarvest[]): {
  logos: RankedGenomaLogoHarvest[];
