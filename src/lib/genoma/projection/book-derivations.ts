/**
 * Derivados del libro de estilo Genoma: especificaciones de color, contraste y tipografía.
 */

import type { GenomaBookView } from "./book-view";
import { formatCmyk, formatRgb, hexToRgb, rgbToCmyk } from "@/app/spaces/genoma/face-utils";
...
export type GenomaBookDerivationsOptions = {
  soloValidado?: boolean;
  logoImageUrl?: string | null;
};

export type GenomaColorSpec = {
  role: string;
  hex: string;
  name?: string;
...
export type GenomaTypographicStep = {
  token: string;
  sizePx: number;
  lineHeightPx: number;
  sample: string;
...
export type GenomaBookDerivations = {
  palette: GenomaColorSpec[];
  wcagMatrix: GenomaWcagPair[];
  typographicScale: GenomaTypographicStep[];
...
export function buildGenomaBookDerivations(
  view: GenomaBookView,
  options: GenomaBookDerivationsOptions = {},
): GenomaBookDerivations {
  const soloValidado = options.soloValidado ?? false;
  const palette: GenomaColorSpec[] = view.palette
    ...
    .filter(Boolean) as GenomaColorSpec[];

  const hexes = palette.map((p) => p.hex);
  const wcagMatrix: GenomaWcagPair[] = [];
  ...
  const typographicScale: GenomaTypographicStep[] = [
...
export function formatColorSpecRow(spec: GenomaColorSpec): string {
