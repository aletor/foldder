/**
 * Render HTML del libro de estilo Genoma (full-bleed, capítulos con derivados).
 */
...
import { buildBookView, type FaceState, type GenomaBookView, type MultiItem, type TraitSlot } from "./book-view";
import { buildGenomaBookDerivations, formatColorSpecRow } from "./book-derivations";
import {
...
import {
  ...
  resolveGenomaStyleGuideSoloValidado,
} from "./style-guide-export-types";

export type GenomaStyleGuideDocument = {
  html: string;
...
function primaryColorHex(view: GenomaBookView, soloValidado: boolean): string {
...
function typographyStylesheets(view: GenomaBookView, soloValidado: boolean, forPdf: boolean): string {
  if (forPdf) return "";
...
function typographyInlineFontFaces(view: GenomaBookView, soloValidado: boolean): string {
  const rules: string[] = [];
  for (const slot of [view.typography.primary, view.typography.secondary]) {
...
function renderPalette(view: GenomaBookView, soloValidado: boolean): string {
...
function renderColorSystem(derivations: ReturnType<typeof buildGenomaBookDerivations>): string {
...
function renderTypography(
  view: GenomaBookView,
  derivations: ReturnType<typeof buildGenomaBookDerivations>,
  soloValidado: boolean,
): string {
  const blocks: string[] = [];
...
function renderVoice(view: GenomaBookView, soloValidado: boolean): string {
  const parts: string[] = [];
...
function renderVisual(view: GenomaBookView, soloValidado: boolean, embedded: Map<string, string>): string {
  const cards: string[] = [];
...
function collectImageUrls(view: GenomaBookView, genome: Genome, soloValidado: boolean): string[] {
  const urls: string[] = [];
...
export async function renderGenomaStyleGuide(
  genome: Genome,
  options: {
    exportMode?: GenomaStyleGuideExportMode;
    projectName?: string;
...
): Promise<GenomaStyleGuideDocument> {
  const exportMode: GenomaStyleGuideExportMode = options.exportMode === "cliente" ? "cliente" : "operativo";
  const soloValidado = resolveGenomaStyleGuideSoloValidado(exportMode);
  ...
  const derivations = buildGenomaBookDerivations(view, {
    soloValidado,
  ...
  const projectName = options.projectName?.trim() || "Genoma";
  const generatedAt = options.generatedAt ?? new Date().toISOString();
  const modeLabel = GENOMA_STYLE_GUIDE_EXPORT_MODE_LABELS[exportMode];
...
export function genomaStyleGuideFilename(projectName: string | undefined, generatedAt: string): string {
  const slug = (projectName ?? "genoma").replace(/\W+/g, "-").toLowerCase();
  const date = generatedAt.slice(0, 10);
  return `${slug}-libro-${date}.pdf`;
}

/** Fallback cliente: descarga HTML cuando Chromium no está disponible. */
export async function downloadGenomaStyleGuideHtml(
  genome: Genome,
  projectName?: string,
  exportMode?: GenomaStyleGuideExportMode,
): Promise<void> {
  const doc = await renderGenomaStyleGuide(genome, { projectName, exportMode });
  const blob = new Blob([doc.html], { type: "text/html;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `${(projectName ?? "genoma").replace(/\W+/g, "-").toLowerCase()}-libro.html`;
  a.click();
