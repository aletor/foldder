...
import { crownedGenome, GENOMA_FIXTURES } from "../fixtures";
import { projectGenomeToBrandKit, projectGenomeToDataset } from "./exports";
import { renderGenomaStyleGuide } from "./style-guide-render";
...
describe("projectGenomeToDataset — inventario completo", () => {
  ...
  it("marca meta.crowned en filas coronadas vs propuestas", () => {
    ...
    const proposed = projectGenomeToDataset("n", GENOMA_FIXTURES.proposed());
...
describe("projectGenomeToBrandKit — solo coronado", () => {
  ...
  it("propuesto sin corona ⇒ tagline null en brand", () => {
    const brand = projectGenomeToBrandKit(GENOMA_FIXTURES.proposed());
    expect(brand.tagline).toBeNull();
  });
});

describe("renderGenomaStyleGuide — modos de export", () => {
  it("modo cliente oculta badges de propuesto en HTML", async () => {
    const genome = GENOMA_FIXTURES.proposed();
    const operativo = await renderGenomaStyleGuide(genome, { exportMode: "operativo" });
    const cliente = await renderGenomaStyleGuide(genome, { exportMode: "cliente" });
  ...
  it("modo operativo incluye capítulos de voz y paleta", async () => {
    const doc = await renderGenomaStyleGuide(crownedGenome(), { exportMode: "operativo", projectName: "Test" });
  ...
  it("modo cliente incluye rasgos coronados completos", async () => {
    const doc = await renderGenomaStyleGuide(crownedGenome(), { exportMode: "cliente", projectName: "OARO" });
