/**
 * Proyección GenomaDocument (slots v2) → Genome (traits) para libro de estilo / PDF.
...
function slot<T>(doc: GenomaDocument, id: SlotId): SlotState<T> | undefined {
  return doc.slots[id] as SlotState<T> | undefined;
}

function mapSources(doc: GenomaDocument): TraitSourceRef[] {
...
function upsertSingleTrait<T>(
  ...
  const userSignal =
    slotState.provenance?.type === "user_input"
      ...
      : [signal("brand-manual", { detail: slotState.provenance?.detail ?? "genoma-v2" })];
...
export function genomaDocumentToGenome(doc: GenomaDocument): Genome {
