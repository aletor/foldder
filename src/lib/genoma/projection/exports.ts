...
export type GenomaBrandProjection = ProjectBrandKit & {
  tagline: string | null;
  toneTraits: string[];
...
export function projectGenomeToBrandKit(genome: Genome): GenomaBrandProjection {
...
export type GenomaDatasetRowMeta = {
  traitId: TraitId;
  crowned: boolean;
  sourceIds: string[];
};

export type GenomaDatasetConstant = {
  fieldId: string;
  constantId: string;
  text?: string;
  color?: string;
  imageUrl?: string;
  role?: string;
  meta: GenomaDatasetRowMeta;
};

export type GenomaDatasetMessageRow = {
  rowId: string;
  message: string;
  audience?: string;
  meta: GenomaDatasetRowMeta;
...
export type GenomaDatasetForbiddenClaimRow = {
  rowId: string;
  text: string;
  why: string;
  meta: GenomaDatasetRowMeta;
...
export type GenomaDatasetSourceRow = {
  rowId: string;
  label: string;
  kind: SourceKind;
  url?: string;
...
export type GenomaDatasetProjection = {
  nodeId: string;
  constants: GenomaDatasetConstant[];
  lists: {
    messages: GenomaDatasetMessageRow[];
    tone: string[];
    forbiddenClaims: GenomaDatasetForbiddenClaimRow[];
    sources: GenomaDatasetSourceRow[];
  };
  toneTraits: string[];
  tagline: string | null;
...
function traitMeta(genome: Genome, traitId: TraitId, crowned: boolean): GenomaDatasetRowMeta {
  const trait = genome.traits[traitId];
...
export function projectGenomeToDataset(nodeId: string, genome: Genome): GenomaDatasetProjection {
  const view = buildBookView(genome);
  const constants: GenomaDatasetConstant[] = [];
  ...
  const messages: GenomaDatasetMessageRow[] = [];
  ...
  const forbiddenClaims: GenomaDatasetForbiddenClaimRow[] = view.voice.claimsForbidden.items.map((item, idx) => ({
  ...
  const sources: GenomaDatasetSourceRow[] = genome.sources.map((s) => ({
    rowId: s.id,
    label: s.label,
    kind: s.kind,
    url: s.kind === "url" ? s.label : undefined,
