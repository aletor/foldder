...
vi.mock("./genoma-brand-board-vision", () => ({
...
vi.mock("./ingest/upload-genoma-file", () => ({
  uploadGenomaIngestFile: vi.fn(async () => ({
    key: "knowledge-files/u/genoma/ingest/test.png",
    url: "/api/spaces/s3-file?key=knowledge-files%2Fu%2Fgenoma%2Fingest%2Ftest.png",
    fileId: "knowledge-files/u/genoma/ingest/test.png",
  })),
}));
...
describe("extractBrandBoardVisualsFromImage paid policy", () => {
  ...
  it("hace una llamada Gemini si el primer pase basta", async () => {
    ...
    const { extractBrandBoardVisualsFromImage } = await import("./genoma-brand-board-image");
