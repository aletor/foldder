import type { GenomaDocument, PaletteValue, TypographyValue } from "./genoma-types";
import {
  buildFontStack,
  cssFontFamilyName,
...
export function isPaletteThemeSource(doc: GenomaDocument): boolean {
  const slot = doc.slots.palette;
  return Boolean(slot.locked || slot.status === "resolved");
}

export function deriveBrandThemeFromDoc(doc: GenomaDocument): BrandThemeResult {
