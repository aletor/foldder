...
import type { GenomaSourceDocKind } from "@/lib/genoma/ingest/genoma-source-pdf-store";

export async function renderLogoAdjustPage(input: {
  source: { buffer: Buffer; kind: GenomaSourceDocKind };
  pageNumber: number;
...
export type LogoAdjustPagePayload = {
  ...
  sourceKind: GenomaSourceDocKind;
};
