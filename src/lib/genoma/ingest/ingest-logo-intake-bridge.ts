/**
 * Fase C — puente logo-intake → candidatos Genoma v2 (ingest PDF deck/manual).
 * Sustituye page-vision para logos cuando GENOMA_LOGO_INTAKE_PDF=1.
 */
...
import { saveBatchDocs } from "@/lib/genoma/logo-intake/batch-store";
import { renderCandidateAdjusted } from "@/lib/genoma/logo-intake/crop";
import { runLogoIntakePipeline } from "@/lib/genoma/logo-intake/pipeline";
import type { IntakeDocInput, IntakePageSelector } from "@/lib/genoma/logo-intake/render";
...
export function isGenomaLogoIntakePdfEnabled(): boolean {
  return process.env.GENOMA_LOGO_INTAKE_PDF === "1";
}
...
async function mapLogoIntakeCandidateToGenoma(input: {
  candidate: LogoCandidate;
  batchId: string;
  fileName: string;
...
export async function extractLogoCandidatesFromPdfLogoIntake(input: {
  ...
  for (let index = 0; index < ordered.length; index += 1) {
    candidates.push(
      await mapLogoIntakeCandidateToGenoma({
