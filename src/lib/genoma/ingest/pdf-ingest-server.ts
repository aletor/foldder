...
import { extractLogoFromPdf, buildBrandCorpusFromGenome, type GenomaLogoCandidate } from "../extractors/logo";
...
import type { GenomaIngestStreamEvent } from "./types";
...
import { persistGenomaSourcePdf } from "./genoma-source-pdf-store";

function* emitApplyResult(result: ApplyExtractionResult): Generator<GenomaIngestStreamEvent> {
...
export async function* ingestPdfIntoGenome(
  ...
  if (opts.userEmail) {
    try {
      await persistGenomaSourcePdf(opts.userEmail, contentSha256, buffer);
    } catch (error) {
      console.warn("[genoma/ingest] persist source pdf failed:", error);
    }
  }
  ...
  let visionPass: GenomaPdfVisionResult | null = null;
  let pageVisionAudit: PageVisionPassRunAudit | null = null;
  let walletCharge = null as Awaited<ReturnType<typeof reserveGenomaIngestAnalysisCharge>>;
  ...
  if (visionGate.willRunVision) {
    try {
      walletCharge = await reserveGenomaIngestAnalysisCharge({
    ...
    try {
      if (phaseAEnabled) {
        ...
        if (auditFixture) {
          if (auditFixture.contentSha256 !== contentSha256) {
            console.warn(
              `[genoma/ingest] pageVisionAuditFixture sha mismatch: audit=${auditFixture.contentSha256.slice(0, 16)} pdf=${contentSha256.slice(0, 16)}`,
            );
          }
          ...
          pageVisionAudit = nivel1
            ? await runPageVisionPassNivel1ForPdf({
                ...
                fileName,
                userEmail: opts.userEmail,
                route: "/lib/genoma/ingest/pdf-ingest-server",
                writeAudit: true,
              })
          ...
          await settleGenomaIngestAnalysisCharge(walletCharge, "pdf");
        ...
        visionPass = await getOrRunGenomaPdfVisionPass({
          buffer,
          contentSha256,
          maxPages: 6,
          userEmail: opts.userEmail,
        ...
        if (visionPass) {
          ...
          await releaseGenomaIngestAnalysisCharge(walletCharge, new Error("vision pass returned null"));
          yield {
            type: "micro",
      ...
      logVisionApiFailed(error);
      await releaseGenomaIngestAnalysisCharge(walletCharge, error);
      if (phaseAEnabled) {
        source = {
          ...source,
          pageVisionPass: skippedPageVisionPassMeta({
  ...
  if (skipFallbackLogo) {
    console.info(
      "[genoma/ingest] skip_classic_logo_fallback",
      JSON.stringify({
        fileName,
  ...
  try {
    ...
    if (!logoResolved && !hasCrownedLogoPrimary(genome)) {
      ...
      let primaryLogos: GenomaLogoCandidate[] = [];
      let logos: GenomaLogoCandidate[] = [];
      ...
      yield {
        ...
        section: "logo",
        preview: sectionPreviewFromGenome(genome, "logo"),
        micro: "Logo de marca ya definido en el genoma",
      };
    }
  } catch {
    yield {
...
export async function ingestImageIntoGenome(
...
): Promise<{ events: GenomaIngestStreamEvent[]; genome: Genome }> {
  ...
  const events: GenomaIngestStreamEvent[] = [
