...
import type { GenomaIngestFileCostHint } from "./genoma-ingest-cost-estimate";

const TEXT_SAMPLE_MAX = 4_000;

export type GenomaIngestFileBuffer = {
  name: string;
  mime: string;
  buffer: Buffer;
};

export async function buildGenomaIngestFileHintsFromBuffers(
  files: GenomaIngestFileBuffer[],
): Promise<GenomaIngestFileCostHint[]> {
  return Promise.all(
