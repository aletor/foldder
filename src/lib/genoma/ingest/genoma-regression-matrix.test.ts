 ...
 *
 * Gate honesto de ingesta real: `genoma-real-ingest-gate.test.ts` (sin pageVisionAuditFixture).
...
describe("genoma regression matrix (Fase B · audits cacheados)", () => {
    ...
    describe.skipIf(!hasPdf(row))(row.id, () => {
      ...
      if (row.id === "oaro-deck") {
        it(
          ...
          async () => {
            const prev = process.env.GENOMA_PAGE_VISION_PASS_ENABLED;
            ...
            try {
              ...
              if (prev === undefined) delete process.env.GENOMA_PAGE_VISION_PASS_ENABLED;
              else process.env.GENOMA_PAGE_VISION_PASS_ENABLED = prev;
...
describe("nivel1 evidence (solo mediciones reales)", () => {
  ...
  )("docs/genoma-evidence/nivel1-ingest-metrics.json existe tras corrida medida", () => {
    const evidencePath = path.join(process.cwd(), "docs/genoma-evidence/nivel1-ingest-metrics.json");
