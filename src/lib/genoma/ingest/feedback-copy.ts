import type { GenomaIngestSectionId } from "./types";
...
export function copyConsolidatingGenome(fileCount: number): string {
  return fileCount === 1
    ? "Consolidando tu genoma…"
    : `Consolidando tu genoma · ${fileCount} documentos`;
}
...
export function copySectionRunning(section: GenomaIngestSectionId): string {
  switch (section) {
    case "palette":
...
export const COPY_GENOME_COMPLETE = "Tu genoma está completo";
