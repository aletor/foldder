...
import type { GenomaVisionLogoHint, GenomaVisionNormalizedBbox } from "./pdf-vision-types";
...
export function bboxIfCornerFormat(bbox: GenomaVisionNormalizedBbox): GenomaVisionNormalizedBbox | null {
  const x0 = bbox.x;
  const y0 = bbox.y;
  const x1 = bbox.width;
...
export function visionBboxCandidates(raw: GenomaVisionNormalizedBbox): GenomaVisionNormalizedBbox[] {
  const seen = new Set<string>();
  const out: GenomaVisionNormalizedBbox[] = [];
  const push = (b: GenomaVisionNormalizedBbox) => {
...
export function writeVisionLogoDebugCrop(
  ...
  if (process.env.GENOMA_VISION_LOGO_DEBUG !== "1") return undefined;
  const dir = process.env.GENOMA_VISION_LOGO_DEBUG_DIR?.trim() || "/tmp";
  fs.mkdirSync(dir, { recursive: true });
  const stable = path.join(dir, `genoma-logo-${label}-p${pageNumber}-a${attempt}.png`);
  const stamped = path.join(dir, `genoma-logo-${label}-p${pageNumber}-a${attempt}-${Date.now()}.png`);
...
export type VisionLogoBboxAttempt = {
  hint: GenomaVisionLogoHint;
  normalized: GenomaVisionNormalizedBbox;
  pixelBBox: PixelBBox;
};
