/**
 * Registro acumulativo «tu genoma · consolidado» — proyección estable del genoma
 * actual para el panel izquierdo. No se reinicia entre subidas: refleja lo que ya
 * vive en el genoma (propuesto o coronado).
 */
...
import { GENOMA_INGEST_SECTION_ORDER } from "./types";
...
export function buildConsolidatedFromGenome(genome: Genome): Record<GenomaIngestSectionId, ConsolidatedRowState> {
  ...
  const rows: Record<GenomaIngestSectionId, ConsolidatedRowState> = {
    palette: {
