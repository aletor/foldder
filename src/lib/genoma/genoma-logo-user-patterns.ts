...
export type GenomaLogoUserPattern = {
...
function patternsObjectKey(userEmail: string): string {
  return buildUserAssetObjectKey({
    userEmail,
    folder: "genoma/logo-patterns",
    filename: "confirmed.json",
    unique: false,
  });
}

export async function loadGenomaLogoUserPatterns(userEmail: string): Promise<GenomaLogoUserPattern[]> {
  if (!userEmail.trim()) return [];
  try {
    const raw = await getFromS3(patternsObjectKey(userEmail));
    const parsed = JSON.parse(raw.toString("utf8")) as { patterns?: GenomaLogoUserPattern[] };
...
export async function recordGenomaLogoUserPattern(
  userEmail: string,
  input: {
    contentSha256?: string;
    pageNumber: number;
  ...
  const next: GenomaLogoUserPattern = {
  ...
  const existing = await loadGenomaLogoUserPatterns(userEmail);
...
export function boostScoreForUserPatterns(
  bboxPage: BBoxPage,
  pageNumber: number,
  patterns: GenomaLogoUserPattern[],
  contentSha256?: string,
): number {
