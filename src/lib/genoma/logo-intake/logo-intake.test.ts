...
import { box2dToBBoxPage, isValidBox2d } from "@/lib/genoma/logo-intake/bbox";
import { dHashHamming, phashNear } from "@/lib/genoma/logo-intake/phash";

const MODULE_DIR = path.join(process.cwd(), "src/lib/genoma/logo-intake");
...
describe("logo-intake", () => {
  ...
  it("no importa capas congeladas del logo-lab clásico", () => {
    const importPattern = /from\s+["']@\/lib\/genoma\/(?:ingest\/)?([^"']+)["']/g;
