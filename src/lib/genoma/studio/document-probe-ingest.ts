/**
 * Puente document-probe → artefactos de runGenomaIngest (slots).
 */
...
import { persistGenomaSourcePdf, persistGenomaSourceRaster } from "../ingest/genoma-source-pdf-store";
import { uploadGenomaIngestFile } from "../ingest/upload-genoma-file";
import { estimateGeminiUsd } from "@/lib/pricing-config";
import { runGenomaDocumentProbe } from "./document-probe";
import type {
...
function normLogoToBBoxPage(logo: Pick<GenomaDocumentProbeLogo, "x" | "y" | "width" | "height">): BBoxPage {
  return [logo.x, logo.y, logo.x + logo.width, logo.y + logo.height];
}

function mergeTypographyFamilies(
  probeTypography: GenomaDocumentProbeTypography[],
  pdfFamilies: string[],
): string[] {
...
async function uploadGalleryCrop(input: {
  jpegBase64: string;
  bbox: Pick<GenomaDocumentProbeOtherImage, "x" | "y" | "width" | "height">;
  userEmail: string;
  fileName: string;
  description: string;
  page: number | null;
