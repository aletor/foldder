...
import {
  ...
  isAllowedCrawlUrl,
  normalizeGenomaUrlInput,
  normalizeHttpUrl,
} from "./url-utils";

describe("genoma url-utils", () => {
  it("normalizes bare domains", () => {
    const parsed = normalizeGenomaUrlInput("coca-cola.com/es/es");
    expect(parsed.ok).toBe(true);
    if (parsed.ok) expect(parsed.url).toBe("https://coca-cola.com/es/es");
  });

  it("normalizes www without scheme", () => {
    const parsed = normalizeGenomaUrlInput("www.coca-cola.com/es/es");
    expect(parsed.ok).toBe(true);
