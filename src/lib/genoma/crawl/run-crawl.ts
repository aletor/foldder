...
import type { EssenceValue, GalleryValue, LegacyOnelinerValue, LegacyValuesValue, Provenance, SlotState, VisualWorldValue } from "../genoma-types";
...
async function fetchText(url: string, timeoutMs: number, maxBytes: number, rootUrl?: string): Promise<string> {
  ...
  const res = await axios.get<string>(url, {
    ...
    responseType: "text",
    headers: {
      "User-Agent": GENOMA_CRAWL_USER_AGENT,
...
export async function fetchCrawlPages(
  ...
  emit?: (event: GenomaStreamEvent) => void,
...
export async function* runGenomaCrawl(
  rootUrl: string,
  jobId = randomUUID(),
  ...
  const collectedEvents: GenomaStreamEvent[] = [];
  const captureEmit = (event: GenomaStreamEvent) => {
  ...
  const logoSignals = [];
  const paletteColors: { hex: string; provenance: import("../genoma-types").Provenance; weight?: number; varName?: string }[] = [];
  const fontFamilyGroups: string[][] = [];
  const harvestedRaw: { url: string; provenance: import("../genoma-types").Provenance; score: number }[] = [];
  ...
  let synthesisInput: import("../llm/genoma-llm-synthesis").GenomaSynthesisInput = {
    corpus,
  ...
  const applyDeterministicCopy = function* (reason: string): Generator<GenomaStreamEvent> {
  ...
  if (!llmEnabled) {
    ...
    const batch = await synthesizeGenomaBatch(synthesisInput);
