...
import { isSemanticTextSlot } from "./genoma-reconcile";
...
export function extractSemanticEvidence(slotId: SlotId, value: unknown): GenomaEvidence[] {
...
export function countSupplementalObservations(slots: GenomaDocument["slots"]): number {
...
export function applyLockedSlotPolicy(
  ...
  if (slotId === "gallery" && patch.value && value) {
    const currentGallery = value as import("./genoma-types").GalleryValue;
    const incomingGallery = patch.value as import("./genoma-types").GalleryValue;
