...
function ownerHashFromUserScopedKey(key: string): string | null {
  ...
  // Legacy brandKit ingest (pre user-assets): knowledge-files/{ownerHash}/brandKit/...
  const legacyBrandKit = key.match(/^knowledge-files\/([a-f0-9]{20})\/brandKit\//);
  if (legacyBrandKit?.[1]) return legacyBrandKit[1];
  return null;
}
