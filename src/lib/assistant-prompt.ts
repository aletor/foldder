...
Optional "executeNodeIds": ordered list of node ids the app should RUN after merging (same as clicking each node’s main action: carousel search, Image Creation generate, Remove Background, Gemini/Grok video, Image Export download). Include every runnable step in pipeline order (sources before targets). Omit or use [] only if the user asked to build the graph without running it.
...
### D — Image Creation (imagen IA) + export
{
  "nodes": [
