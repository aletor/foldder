...
import { persistBrandKitSourcePdf } from "./ingest/brand-kit-source-pdf-store";
import { mapVisionLogoEntryToCandidate } from "./brand-kit-pdf-logo-vision";
import {
  extractLogoCandidatesFromPdfLogoIntake,
  isBrandKitLogoIntakePdfEnabled,
...
export { isLikelyBrandManualPdf } from "./brand-kit-pdf-brand-manual-detect";
...
export async function extractBrandManualVisualsFromPdf(input: {
  ...
  if (isBrandKitLogoIntakePdfEnabled()) {
    try {
      ...
      console.warn("[brandKit:manual-logo-intake]", error instanceof Error ? error.message : error);
    }
  }
