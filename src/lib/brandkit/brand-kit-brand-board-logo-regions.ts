...
import { segmentContrastLogoRegions } from "./brand-kit-logo-segment-regions";
import {
  boostScoreForUserPatterns,
  loadBrandKitLogoUserPatterns,
  type BrandKitLogoUserPattern,
} from "./brand-kit-logo-user-patterns";
...
export async function rankBrandBoardLogoRegions(
  ...
  options?: {
    ...
    pageNumber?: number;
    cachedPatterns?: BrandKitLogoUserPattern[];
  },
): Promise<BrandBoardLogoRegionScore[]> {
  const patterns =
    options?.cachedPatterns ??
    (options?.userEmail ? await loadBrandKitLogoUserPatterns(options.userEmail) : []);
