...
import { needsBrandKitMediaProxy } from "./brand-kit-media-url";
...
export function isPersistedBrandKitMediaUrl(url: string): boolean {
...
export function externalGalleryMediaUrls(gallery: GalleryValue | undefined, limit = 24): string[] {
  ...
  for (const item of gallery.harvested) {
    ...
    if (!source || !needsBrandKitMediaProxy(source) || isPersistedBrandKitMediaUrl(source)) continue;
