...
export const brandKitLocaleEs = {
  ...
  reconcileDecisionAction: "Elige qué versión quieres guardar en el brandKit.",
  ...
  reviewHint: (slotId: string) => {
    ...
    return "Guárdala en el brandKit si encaja, o edítala antes.";
  },
  ...
  reviewAskButton: (count: number) => `BrandKit te pregunta (${count})`,
...
export const confirmLabelForSlot: Partial<Record<SlotId, string>> = {
  logo: brandKitLocaleEs.confirmLogo,
  palette: brandKitLocaleEs.confirmPalette,
  typography: brandKitLocaleEs.confirmTypography,
  ...
  gallery: brandKitLocaleEs.confirmGallery,
};

export function formatProvenanceEs(detail?: string, type?: string): string {
  if (!detail && !type) return "";
  if (type === "user_input") return brandKitLocaleEs.you;
