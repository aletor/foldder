import { box2dToBBoxPage, expandBBoxPage, isValidBox2d } from "@/lib/brandkit/logo-intake/bbox";
import { cropLogoFromFrame, cropLogoFromImageDoc, renderCandidateAdjusted, renderEditPage, trimBBoxPageFromPage } from "@/lib/brandkit/logo-intake/crop";
import { mapPool } from "@/lib/brandkit/logo-intake/concurrency";
import { computeDHashHex, phashNear } from "@/lib/brandkit/logo-intake/phash";
import {
...
import { scoreLogoQuality } from "@/lib/brandkit/logo-intake/quality";
import type { IntakeDocInput, IntakeFrame, IntakePageSelector } from "@/lib/brandkit/logo-intake/render";
import type { LogoCandidate, LogoProposal } from "@/lib/brandkit/logo-intake/types";
import type { ParsedVisionBrandColorRegion, ParsedVisionLogo, ParsedVisionResponse } from "@/lib/brandkit/logo-intake/vision-schema";
import { invokeLogoIntakeVision } from "@/lib/brandkit/logo-intake/vision-invoker";
import { renderIntakeFrames } from "@/lib/brandkit/logo-intake/render";
...
export async function runLogoIntakePipeline(input: {
  ...
  const { parsed, visionMs, visionCalls } = await invokeLogoIntakeVision({
    ...
    route: "/api/brandKit/logo-intake/analyze",
