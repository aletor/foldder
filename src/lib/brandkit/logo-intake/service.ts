...
import { logLogoIntakeEvent } from "@/lib/brandkit/logo-intake/events";
import { assertIntakeFileCount, prepareIntakeDoc } from "@/lib/brandkit/logo-intake/ingest-files";
import { registerLockedSightings } from "@/lib/brandkit/logo-intake/locked-sightings";
import { runLogoIntakePipeline } from "@/lib/brandkit/logo-intake/pipeline";
import { renderCandidateHiRes, renderCandidateAdjusted } from "@/lib/brandkit/logo-intake/crop";
import { uploadCanonicalLogoRaster } from "@/lib/brandkit/logo-intake/canonical-asset";
import { bboxAreaDelta, normalizeBBoxPage } from "@/lib/brandkit/logo-intake/bbox-ui";
...
import {
  ...
  loadUndoSnapshot,
  saveUndoSnapshot,
} from "@/lib/brandkit/logo-intake/undo-validate";
import { computeDHashHex } from "@/lib/brandkit/logo-intake/phash";
...
import {
  ...
  writeBrandLogoAsset,
  writeBrandLogoSvg,
} from "@/lib/brandkit/logo-intake/store";
import {
  getBatchManifest,
  getBatchProposal,
  saveBatchDocs,
  saveBatchProposal,
} from "@/lib/brandkit/logo-intake/batch-store";
import type {
  BrandLogoState,
  LogoCandidate,
...
import { vectorizeLogo } from "@/lib/brandkit/logo-intake/vectorize";
...
export async function analyzeLogoIntakeFromDocs(input: {
  ...
  batchId: string;
  userEmail?: string;
  onPipelineEvent?: (event: import("@/lib/brandkit/logo-intake/pipeline").LogoIntakePipelineEvent) => void;
}): Promise<LogoIntakeAnalyzeResult> {
...
export async function getEditPageForCandidate(input: {
  ...
  const started = Date.now();
  const { renderEditPage } = await import("@/lib/brandkit/logo-intake/crop");
