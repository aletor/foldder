...
export const BRAND_KIT_BRAND_LOGO_DDB_TABLE_ENV = "FOLDDER_BRAND_KIT_BRAND_LOGO_DDB_TABLE";
const FILE_STORE_DIR = path.join(process.cwd(), "data/brand-kit-brand-logo");
...
function tableName(): string | null {
  if (!isDynamoEnabled(BRAND_KIT_BRAND_LOGO_DDB_TABLE_ENV)) return null;
  return process.env[BRAND_KIT_BRAND_LOGO_DDB_TABLE_ENV]?.trim() || null;
}

function pk(projectId: string): string {
  return `BRAND_KIT_BRAND_LOGO#${projectId}`;
}

type StoreItem = BrandLogoState & { pk: string; sk: "STATE"; entityType: "brand-kit-brand-logo" };
...
class DynamoBrandLogoStore implements BrandLogoStore {
  ...
  async set(state: BrandLogoState): Promise<void> {
    ...
    const item: StoreItem = {
      ...
      sk: "STATE",
      entityType: "brand-kit-brand-logo",
    };
