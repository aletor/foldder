...
import type { Candidate, LogoValue } from "../brand-kit-types";
import { applyLogoVisionLabels } from "../brand-kit-logo-policy";
import {
...
} from "./brand-kit-llm-validate";

import { estimateGeminiUsd } from "@/lib/pricing-config";
import type { EvidenceCandidate } from "../brand-kit-evidence-candidates";
...
export type BrandKitDocumentProbeContext = {
  textSummary: string[];
...
export type BrandKitSynthesisInput = {
  corpus: string;
...
async function callBrandKitJson<T>(
  input: BrandKitSynthesisInput,
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      console.error(`[brand-kit-llm/${operation}] attempt ${attempt + 1}`, error);
    }
  }
...
export async function synthesizeVoice(input: BrandKitSynthesisInput): Promise<VoiceLlmResponse | null> {
  const signals = cheapCopySignals(input.corpus);
  const raw = await callBrandKitJson(
    input,
    [
...
export async function synthesizeValues(input: BrandKitSynthesisInput): Promise<ValuesLlmResponse | null> {
  const raw = await callBrandKitJson(
    input,
    [
...
export async function synthesizeOnelinerOptions(input: BrandKitSynthesisInput): Promise<OnelinerLlmResponse | null> {
  return callBrandKitJson(
    input,
...
export async function labelLogoCandidatesWithVision(
  ...
  input: Pick<BrandKitSynthesisInput, "userEmail" | "route" | "onLlmCostUsd">,
  ...
  try {
    ...
    console.error("[brand-kit-llm/logo_label]", error);
    return candidates;
  }
}
