...
import type { BrandKitIngestSectionId, BrandKitIngestStreamEvent, BrandKitSectionPreview } from "@/lib/brandkit/ingest/types";
...
import {
  ...
  copyReceivingFiles,
  copyVisitingUrl,
} from "@/lib/brandkit/ingest/feedback-copy";
...
export type ActivitySessionState = {
  active: boolean;
  /** true cuando el brandKit ya tenía material antes de este lote. */
  incremental: boolean;
  phase: "receiving" | "reading" | "discovering" | "done";
  statusLine: string | null;
  receiveCount: number;
  sections: Record<BrandKitIngestSectionId, SectionRowState>;
...
export type BrandKitIngestFeedbackState = {
...
export function emptySectionRows(): Record<BrandKitIngestSectionId, SectionRowState> {
  return Object.fromEntries(
    BRAND_KIT_INGEST_SECTION_ORDER.map((id) => [id, { status: "pending" as const }]),
  ) as Record<BrandKitIngestSectionId, SectionRowState>;
}
...
export function idleIngestFeedback(genome: Genome = emptyGenome()): BrandKitIngestFeedbackState {
  return {
...
const SECTION_LABELS: Record<BrandKitIngestSectionId, string> = {
  palette: "Paleta",
  logo: "Logo",
...
export function sectionLabel(id: BrandKitIngestSectionId): string {
...
type Action =
  ...
  | { type: "stream_event"; event: BrandKitIngestStreamEvent };

export function reduceIngestFeedback(
  state: BrandKitIngestFeedbackState,
  action: Action,
): BrandKitIngestFeedbackState {
  switch (action.type) {
    case "reset":
...
function ensureActivity(state: BrandKitIngestFeedbackState): ActivitySessionState {
  return state.activity ?? { ...emptyActivity(), active: true };
}

function applyStreamEvent(
  state: BrandKitIngestFeedbackState,
  event: BrandKitIngestStreamEvent,
): BrandKitIngestFeedbackState {
  ...
  const next: BrandKitIngestFeedbackState = { ...state, activity: nextActivity };
...
export function consolidatedHasContent(state: BrandKitIngestFeedbackState): boolean {
  return BRAND_KIT_INGEST_SECTION_ORDER.some((id) => state.consolidated[id].status !== "empty");
}

/** Consolidado visible solo tras el primer lote o en ingesta incremental. */
export function shouldShowConsolidatedBox(state: BrandKitIngestFeedbackState): boolean {
