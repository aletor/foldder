...
import { brandKitOperationId } from "../ingest/paid-operations";
...
import { BrandKitVisionPassError } from "./brand-kit-vision-pass-error";
import {
  logVisionApiCall,
...
import {
  BRAND_KIT_PDF_VISION_PASS_VERSION,
  type BrandKitPdfVisionResult,
  type BrandKitVisionLogoHint,
  type BrandKitVisionLogoPolarity,
  type BrandKitVisionNormalizedBbox,
  type BrandKitVisionPageImage,
  type BrandKitVisionPaletteEntry,
  type BrandKitVisionThirdPartyLogo,
  type BrandKitVisionTypographyHint,
  type BrandKitVisionVisualEntry,
...
export type BrandKitVisionPassInvoker = (input: {
  pageImages: BrandKitVisionPageImage[];
  userEmail?: string;
  route?: string;
  operationId: string;
}) => Promise<BrandKitPdfVisionResult | null>;
...
function parseNormalizedBbox(raw: unknown): BrandKitVisionNormalizedBbox | null {
...
function parsePolarity(raw: unknown): BrandKitVisionLogoPolarity | null {
  if (raw === "light_mark" || raw === "dark_mark") return raw;
  return null;
}

function parseLogoHint(raw: unknown): BrandKitVisionLogoHint | null {
  if (!raw || typeof raw !== "object") return null;
...
function parseThirdParty(raw: unknown): BrandKitVisionThirdPartyLogo[] {
  ...
  return raw
    ...
    .filter((v): v is BrandKitVisionThirdPartyLogo => v !== null);
}

function parsePaletteEntry(raw: unknown): BrandKitVisionPaletteEntry | null {
  if (!raw || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  const role = o.role;
...
function parseTypography(raw: unknown): BrandKitVisionTypographyHint | undefined {
  ...
  const hint: BrandKitVisionTypographyHint = {
...
function parseVisual(raw: unknown): BrandKitVisionVisualEntry[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((item) => {
      if (!item || typeof item !== "object") return null;
      ...
      if (typeof category !== "string" || !IMAGE_CATEGORIES.includes(category as BrandKitVisionVisualEntry["category"])) {
        return null;
      }
      ...
      return {
        category: category as BrandKitVisionVisualEntry["category"],
        description,
        ...(Number.isFinite(imageRefIndex) ? { imageRefIndex: Math.round(imageRefIndex) } : {}),
      };
    })
    .filter((v): v is BrandKitVisionVisualEntry => v !== null);
}

export function parseBrandKitPdfVisionJson(raw: unknown): BrandKitPdfVisionResult | null {
  if (!raw || typeof raw !== "object") return null;
  const root = raw as Record<string, unknown>;
  const logoRoot = root.logo;
  let logo: BrandKitPdfVisionResult["logo"];
  ...
  const palette = Array.isArray(root.palette)
    ? root.palette.map(parsePaletteEntry).filter((v): v is BrandKitVisionPaletteEntry => v !== null)
    : [];
  ...
  return {
    version: BRAND_KIT_PDF_VISION_PASS_VERSION,
    logo,
    palette,
    typography,
    visual,
...
export async function defaultBrandKitVisionPassInvoker(input: {
  pageImages: BrandKitVisionPageImage[];
  userEmail?: string;
  route?: string;
  operationId: string;
  ...
  if (!apiKey) {
    logVisionNoApiKey();
    throw new BrandKitVisionPassError(
      "Análisis visual no disponible: falta configurar GEMINI_API_KEY en el servidor.",
    );
  }
  if (!input.pageImages.length) {
    throw new BrandKitVisionPassError("No se pudieron renderizar páginas para el análisis visual.");
  }
  ...
  await recordApiUsage({
    ...
    route: input.route ?? "/lib/brandKit/ingest/pdf-vision-pass",
  ...
  const result = parseBrandKitPdfVisionJson(parsed);
  if (!result) {
...
export type RunBrandKitPdfVisionPassInput = {
  buffer: Buffer;
  contentSha256: string;
  maxPages?: number;
  userEmail?: string;
  route?: string;
  invokeVision?: BrandKitVisionPassInvoker;
  pageImagesOverride?: BrandKitVisionPageImage[];
  skipCache?: boolean;
  /** Si true, falla en lugar de devolver null (ingesta de pago autorizada). */
  requireResult?: boolean;
};

export async function getOrRunBrandKitPdfVisionPass(
  input: RunBrandKitPdfVisionPassInput,
): Promise<BrandKitPdfVisionResult | null> {
  ...
  const operationId = brandKitOperationId("ingest", input.contentSha256.slice(0, 32));
  const invoke = input.invokeVision ?? defaultBrandKitVisionPassInvoker;
  let result: BrandKitPdfVisionResult | null = null;
  try {
    result = await invoke({
      pageImages,
      userEmail: input.userEmail,
  ...
  const normalized = { ...result, version: BRAND_KIT_PDF_VISION_PASS_VERSION };
  setCachedPdfVisionPass(cacheKey, normalized);
  return normalized;
}

export function typographyGuessFromVisionPass(
  typography: BrandKitVisionTypographyHint | undefined,
