...
import { loadBrandKitSourcePdf } from "@/lib/brandkit/ingest/brand-kit-source-pdf-store";
import { renderPdfPageCrop } from "@/lib/brain/pdf-page-render";
import {
  resolveHiResLogoRasterForVectorize,
  VECTORIZE_RERASTER_DPI,
} from "@/lib/brandkit/ingest/vectorize-hires-raster";

describe("resolveHiResLogoRasterForVectorize", () => {
  beforeEach(() => {
    vi.mocked(loadBrandKitSourcePdf).mockReset();
  ...
  it("usa fallback cuando falta vectorSource completo", async () => {
    ...
    expect(loadBrandKitSourcePdf).not.toHaveBeenCalled();
  });

  it("re-rasteriza desde PDF fuente a 600 DPI con bbox escalado", async () => {
    const fallback = Buffer.from("lowres");
    const hiRes = Buffer.from("hires");
    vi.mocked(loadBrandKitSourcePdf).mockResolvedValue(Buffer.from("pdf"));
    ...
    expect(loadBrandKitSourcePdf).toHaveBeenCalledWith("u@test.com", "abc123");
  ...
  it("cae a fallback si el PDF fuente no está en S3", async () => {
    const fallback = Buffer.from("lowres");
    vi.mocked(loadBrandKitSourcePdf).mockResolvedValue(null);
