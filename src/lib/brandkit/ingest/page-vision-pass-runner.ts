...
import {
  BRAND_KIT_PAGE_VISION_NIVEL1_VERSION,
...
/** Gate único de Fase A — solo `BRAND_KIT_PAGE_VISION_PASS_ENABLED=1` (sin heurística por filename). */
export function isPageVisionPassEnabled(): boolean {
  return process.env.BRAND_KIT_PAGE_VISION_PASS_ENABLED === "1";
}
...
export type PageVisionIngestMetrics = {
  ...
  /** Tiempo hasta brandKit interactivo (sin harvest nativo). */
...
export type PageVisionPassRunAudit = {
  version: typeof BRAND_KIT_PAGE_VISION_PASS_VERSION;
  ...
  nivel1Contract?: typeof BRAND_KIT_PAGE_VISION_NIVEL1_VERSION;
  ...
  for (const pageNumber of plan.selected) {
    ...
    if (!audit.ok) {
      ...
      audit = await analyzeOnePage({
        ...
        pngBase64,
        userEmail: input.userEmail,
        route: input.route ?? "/lib/brandKit/ingest/page-vision-pass-retry",
