...
import type { BrandKitPdfVisionResult, BrandKitVisionPageImage } from "./pdf-vision-types";

const mockPage: BrandKitVisionPageImage = {
  mimeType: "image/png",
  base64: "aGk=",
  pageNumber: 1,
  width: 800,
...
describe("parseBrandKitPdfVisionJson", () => {
  it("parsea logo, paleta, tipografía y visual", () => {
    const parsed = parseBrandKitPdfVisionJson({
      logo: {
        emitter: {
          page: 1,
...
describe("getOrRunBrandKitPdfVisionPass", () => {
  beforeEach(() => {
    clearPdfVisionCacheForTests();
  });

  it("un solo pase por documento — cache por contentSha256", async () => {
    const invokeVision = vi.fn(async (): Promise<BrandKitPdfVisionResult> => ({
    ...
    const first = await getOrRunBrandKitPdfVisionPass(input);
    const second = await getOrRunBrandKitPdfVisionPass(input);
  ...
  it("no cachea fallos — reintenta tras null", async () => {
    const invokeVision = vi
      ...
      .mockResolvedValueOnce({
        ...
        confidence: 0.5,
        provider: "mock",
      } satisfies BrandKitPdfVisionResult);
