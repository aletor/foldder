...
export function buildBrandKitIngestObjectKey(userEmail: string, filename: string, mime: string): string {
  ...
  return buildUserAssetObjectKey({
    userEmail,
    folder: "brandKit/ingest",
    filename: `${stem}-${id}.${ext}`,
    unique: false,
  });
}

export async function uploadBrandKitIngestFile(args: {
  userEmail: string;
  filename: string;
  mime: string;
  buffer: Buffer;
}): Promise<{ key: string; url: string; fileId: string }> {
  const key = buildBrandKitIngestObjectKey(args.userEmail, args.filename, args.mime);
