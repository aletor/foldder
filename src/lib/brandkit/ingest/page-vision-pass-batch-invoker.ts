...
async function recordBatchUsage(input: {
  ...
  await recordApiUsage({
    ...
    route: input.route ?? "/lib/brandKit/ingest/page-vision-pass-nivel1",
...
function parseBatchResponse(response: unknown, mode: "json_schema" | "tool_calling"): unknown {
  if (mode === "tool_calling") {
    ...
    if (!raw || typeof raw !== "object") {
      throw new BrandKitVisionPassError("Batch Fase A sin tool utilizable.");
    }
    return raw;
  }
  ...
  if (!raw || typeof raw !== "object") {
    throw new BrandKitVisionPassError("Batch JSON schema sin objeto raíz parseable.");
  }
  return raw;
}
...
async function callBatchJsonSchema(input: {
  ...
  try {
    ...
    const err = new BrandKitVisionPassError(error instanceof Error ? error.message : String(error));
...
export async function invokePageVisionPassBatchModel(input: {
  ...
  if (!input.pages.length) {
    throw new BrandKitVisionPassError("Batch Fase A vacío.");
  }
  ...
  const tryAttempt = async (
    ...
    for (let attemptIndex = 0; attemptIndex < retries; attemptIndex += 1) {
      ...
      try {
        ...
        lastError = error;
        if (error instanceof BrandKitVisionPassError && error.billedUsage) {
          llmCalls += 1;
  ...
  if (!raw) {
    throw lastError instanceof Error
      ? lastError
      : new BrandKitVisionPassError("Batch Fase A sin respuesta utilizable tras reintentos.");
  }
