...
export const BRAND_KIT_PAGE_VISION_PASS_VERSION = "2026-07-07-page-structured-5";

/** Contrato batch Nivel 1 (slim) — bump al cambiar schema/prompt de ingesta. */
export const BRAND_KIT_PAGE_VISION_NIVEL1_VERSION = "2026-07-07-nivel1-slim-7";
...
export function pageVisionNivel1GeminiModel(): string {
  return (
    process.env.BRAND_KIT_NIVEL1_GEMINI_MODEL?.trim() ||
...
export function pageVisionNivel1CacheKey(contentSha256: string, pageNumber: number): string {
  ...
  return `brandKit:nivel1:${BRAND_KIT_PAGE_VISION_NIVEL1_VERSION}:edge${PAGE_VISION_NIVEL1_MAX_LONG_EDGE}:${sha}:p${pageNumber}`;
}
...
export function pageVisionPassCacheKey(contentSha256: string, pageNumber: number): string {
  const sha = contentSha256.trim().slice(0, 64);
  return `brandKit:page-vision:${BRAND_KIT_PAGE_VISION_PASS_VERSION}:dpi${PAGE_VISION_PASS_DPI}:${sha}:p${pageNumber}`;
}
