/**
 * Fixtures de brandKit para la cara estática (§ punto 3): ghost → proposed → crowned.
 *
 * Se construyen con los reductores reales (`createCandidate`/`createTrait`/`crown`)
 * para que sean BrandKits VÁLIDOS, no maquetas. Las imágenes son data-URIs SVG
...
/** BrandKit vacío: todo ghost. */
export function ghostGenome(): Genome {
  return emptyGenome();
}

/** BrandKit con propuestas SIN coronar (estado proposed). */
...
/** BrandKit resuelto: rasgos principales coronados (estado crowned). */
...
export const BRAND_KIT_FIXTURES = {
  ghost: ghostGenome,
  proposed: proposedGenome,
  crowned: crownedGenome,
} as const;

export type BrandKitFixtureName = keyof typeof BRAND_KIT_FIXTURES;
