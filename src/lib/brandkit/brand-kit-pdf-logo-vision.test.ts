...
vi.mock("./ingest/brand-kit-source-pdf-store", () => ({
  persistBrandKitSourcePdf: vi.fn(async () => "brandKit/sources/sha-deck/sha-deck-full.pdf"),
}));
...
vi.mock("./ingest/ingest-logo-intake-bridge", () => ({
  isBrandKitLogoIntakePdfEnabled: () => logoIntakeEnabled.value,
...
import { extractLogoCandidatesFromDeckPdf } from "./brand-kit-pdf-logo-vision";
...
describe("extractLogoCandidatesFromDeckPdf", () => {
  ...
  it("uses logo-intake pipeline when BRAND_KIT_LOGO_INTAKE_PDF=1", async () => {
  ...
  it("uses page vision pass when logo-intake flag is off", async () => {
    vi.mocked(runPageVisionPassForPdf).mockResolvedValueOnce({
      version: "brand-kit-page-vision-pass-v1",
      dpi: 144,
