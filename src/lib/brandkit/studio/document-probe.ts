/**
 * Modo laboratorio — una sola llamada LLM por documento (sidebar BrandKit Studio).
 */
...
const PROBE_MODEL =
  process.env.BRAND_KIT_DOCUMENT_PROBE_MODEL?.trim() ||
...
function parseTypographyRole(value: unknown): BrandKitDocumentProbeTypographyRole {
  if (value === "display" || value === "heading" || value === "body") return value;
  return "body";
}

function parseTypographyArray(raw: unknown): BrandKitDocumentProbeTypography[] {
  if (!Array.isArray(raw)) return [];
  const rows: BrandKitDocumentProbeTypography[] = [];
...
async function measureBackgroundLightness(
  jpegBase64: string,
  logo: Pick<BrandKitDocumentProbeLogo, "x" | "y" | "width" | "height">,
): Promise<number> {
...
async function enrichLogosWithBackgroundLightness(
  ...
  frames: ProbeFrame[],
): Promise<BrandKitDocumentProbeLogo[]> {
...
function resolvePrimaryLogo(logos: BrandKitDocumentProbeLogo[]): BrandKitDocumentProbeLogo | null {
...
function logoBboxKey(logo: BrandKitDocumentProbeLogo): string {
...
async function refinePrimaryLogoBbox(
  ...
  frames: ProbeFrame[],
): Promise<{ logos: BrandKitDocumentProbeLogo[]; primaryLogo: BrandKitDocumentProbeLogo | null }> {
...
type NormalizedBbox = Pick<BrandKitDocumentProbeLogo, "x" | "y" | "width" | "height">;
...
function parseOtherImagesArray(
  raw: unknown,
  max: number,
): Array<Omit<BrandKitDocumentProbeOtherImage, "thumbnailBase64">> {
  if (!Array.isArray(raw)) return [];
  const otherImages: Array<Omit<BrandKitDocumentProbeOtherImage, "thumbnailBase64">> = [];
  for (const entry of raw) {
    if (!entry || typeof entry !== "object") continue;
    const row = entry as Record<string, unknown>;
...
async function enrichOtherImagesWithThumbnails(
  otherImages: Array<Omit<BrandKitDocumentProbeOtherImage, "thumbnailBase64">>,
  frames: ProbeFrame[],
): Promise<BrandKitDocumentProbeOtherImage[]> {
  const jpegByPage = new Map<number | null, string>();
  for (const frame of frames) jpegByPage.set(frame.pageNumber, frame.jpegBase64);
...
> & {
  ...
  const logos: BrandKitDocumentProbeLogo[] = [];
  ...
  const primaryColors: BrandKitDocumentProbeColor[] = [];
...
async function runExtendedOtherImagesProbe(input: {
  ...
  fileName: string;
  frames: ProbeFrame[];
  knownLogos: BrandKitDocumentProbeLogo[];
  maxImages: number;
}): Promise<Array<Omit<BrandKitDocumentProbeOtherImage, "thumbnailBase64">>> {
...
export async function runBrandKitDocumentProbe(input: {
  buffer: Buffer;
  fileName: string;
  mime: string;
}): Promise<BrandKitDocumentProbeResult> {
