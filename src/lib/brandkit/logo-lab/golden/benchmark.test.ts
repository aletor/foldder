...
import type { GoldenDocument } from "@/lib/brandkit/logo-lab/golden/types";
import {
  computeDocumentDetectionRecall,
  listVisionLogoDetections,
} from "@/lib/brandkit/logo-lab/golden/benchmark";
import { bboxIoU } from "@/lib/brandkit/logo-lab/golden/coords";

vi.mock("@/lib/brandkit/logo-lab/golden/coords", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/brandkit/logo-lab/golden/coords")>();
  return {
    ...actual,
...
describe("vision cache envelope", () => {
  it("parsea audits legacy raw con contentSha256", async () => {
    const { readVisionCacheEnvelope } = await import("@/lib/brandkit/logo-lab/golden/vision-cache");
    const fs = await import("node:fs");
    const path = await import("node:path");
    const { visionCacheDir } = await import("@/lib/brandkit/logo-lab/golden/paths");
