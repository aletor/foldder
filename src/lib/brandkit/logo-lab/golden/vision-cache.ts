...
import { BRAND_KIT_PAGE_VISION_NIVEL1_VERSION } from "@/lib/brandkit/ingest/page-vision-pass-version";
import { visionCacheDir } from "@/lib/brandkit/logo-lab/golden/paths";
...
export function visionCacheKey(contentSha256: string): string {
  return `${contentSha256.trim().slice(0, 64)}__${BRAND_KIT_PAGE_VISION_NIVEL1_VERSION}.audit.json`;
}
...
function parseVisionCacheFile(raw: string): VisionCacheEnvelope | null {
  try {
    ...
    return {
      ...
      pipelineVersion: audit.nivel1Contract ?? BRAND_KIT_PAGE_VISION_NIVEL1_VERSION,
      source: "legacy_raw",
...
export function writeVisionCache(
  ...
  const envelope: VisionCacheEnvelope = {
    version: 1,
    contentSha256: sha,
    pipelineVersion: BRAND_KIT_PAGE_VISION_NIVEL1_VERSION,
    source,
