...
} from "./brand-kit-brand-board-vision-schema";
import {
  BRAND_BOARD_VISION_SYSTEM,
  BRAND_BOARD_VISION_USER_PROMPT,
  BRAND_BOARD_LOGO_FOCUS_USER_PROMPT,
} from "./brand-kit-brand-board-vision-prompt";

const BRAND_BOARD_VISION_MODEL =
  process.env.BRAND_KIT_BRAND_BOARD_VISION_MODEL?.trim() ||
...
export async function invokeBrandBoardVision(
  ...
  const operationId = `brandKit:brand-board:${input.contentSha256.slice(0, 16)}`;
...
export async function invokeBrandBoardLogoFocusVision(
  ...
  const ai = new GoogleGenAI({ apiKey });
  const operationId = `brandKit:brand-board-logo:${input.contentSha256.slice(0, 16)}`;
